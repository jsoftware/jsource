...
static A jtgadv(J jt,A w){A hs;I n;
 ...
 I flag=VASGSAFE+VFIX+VNOSELF;  // where we will build verb flags, inited as if wn<3
 if(wn==3){if(FAV(AAV(hs)[2])->id==CRIGHT)AS(hs)[0]=2; else flag=FAV(AAV(hs)[2])->flag|VFIX+VNOSELF;}  // if v2=], remove it from gerund count in shape (leave in AN for free); if 3 gerunds, init from v2
 flag=(flag&FAV(AAV(hs)[0])->flag&FAV(AAV(hs)[1])->flag&VASGSAFE+VFIX+VNOSELF)+(VGERL)+(alr-2>0?alr-2:alr);  // wn may be 2 or 3
...
DF1(jtamend){F12IP;
 ARGCHK1(w);
 if(unlikely(AT(w)&VERB)) R fdef(0,CRBRACE,VERB,(AF)mergv1,(AF)amccv2,w,0L,0L,FAV(w)->flag&VASGSAFE+VFIX+VNOSELF, RMAX,RMAX,RMAX);  // verb} 
  ...
  R z;
 }else R fdef(0,CRBRACE,VERB,(AF)mergn1,(AF)jtamendn2c,w,0L,0L,VFIX+VNOSELF|VASGSAFE|VIRS2, RMAX,RMAX,RMAX);   // m}"r with IRS
}
