...
static DF1(jtoblique){F12IP;A x,y,z;I m,n,r;
 ARGCHK1(w);
 r=AR(w);  // r = rank of w
 ...
 RZ(x=ATOMIC2(jt,xm,0L,xn,1L,ds(CPLUS))); AR(x)=1; AS(x)[0]=AN(x);
...
// obsolete    adocv=var(ds(CSTAR),at,wt); varps(adocvsum,FAV(f)->fgh[0],wt,0);  // get sum routine from f/, which is +/
   ...
   for(i=0;i<zn;++i){
    ...
    p=MIN(1+i,zn-i); p=MIN(p,k);
    I rc=(adocv.f)(~p,yv,u,v,0,jt); rc=rc<0?EWOVIP+EWOVIPMULII:rc; if(rc&255)jsignal(rc); if(255&(rc=((AHDRRFN*)adocvsum.f)((I)1,p,(I)1,yv,zv,jt)))jsignal(rc);
    ++zv;
   }
...
#define KSLGLPMEAN(wincr,zincr,pwvinit,pwvupd,freqt) \
 ...
 for(indexx=0, indexn=AN(ai), wv=voidAV(w); indexx!=indexn;++indexx, wv=(C*)wv+(wincr)){ \
    ...
    switch(routineid){
    case 3: {I rc; KSLGLP(celllen,celllen,MC(partition,wv,celllen);,ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc)) break;}  // <. >.
    case 7: {I rc; KSLGLP(cellatoms,celllen,DO(cellatoms, ((I*)partition)[i]=((C*)wv)[i];),ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc)) break;}  // + B01
    case 11: {I rc; KSLGLPMEAN(cellatoms,celllen,DO(cellatoms, ((D*)partition)[i]=((B*)wv)[i];),ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),D) break;}  // mean B01
    case 15: {I rc; KSLGLPMEAN(celllen,celllen,DO(cellatoms, ((D*)partition)[i]=((I*)wv)[i];),ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),D) break;}  // mean INT
    case 16: {I rc; KSLGLPMEAN(celllen,celllen,MC(partition,wv,celllen);,ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),I) break;}  // mean for XNUM/RAT
    case 17: {I rc; KSLGLPMEAN(celllen,celllen,MC(partition,wv,celllen);,ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),D) break;}  // mean for FL
...
#define KSLSRLPMEAN(wincr,zincr,pwvinit,pwvupd,freqt) \
 ...
 for(av=IAV(a), wv=voidAV(w); nitems; av=(I*)((I)av+k), wv=(C*)wv+(wincr),--nitems){ \
    ...
    switch(routineid){
    case 3: {I rc; KSLSRLP(celllen,celllen,MC(partition,wv,celllen);,ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc)) break;}  // <. >.
    case 7: {I rc; KSLSRLP(cellatoms,celllen,DO(cellatoms, ((I*)partition)[i]=((C*)wv)[i];),ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc)) break;}  // + B01
    case 11: {I rc; KSLSRLPMEAN(cellatoms,celllen,DO(cellatoms, ((D*)partition)[i]=((B*)wv)[i];),ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),D) break;}  // mean B01
    case 15: {I rc; KSLSRLPMEAN(celllen,celllen,DO(cellatoms, ((D*)partition)[i]=((I*)wv)[i];),ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),D) break;}  // mean INT
    case 16: {I rc; KSLSRLPMEAN(celllen,celllen,MC(partition,wv,celllen);,ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),I) break;}  // mean for XNUM/RAT
    case 17: {I rc; KSLSRLPMEAN(celllen,celllen,MC(partition,wv,celllen);,ASSERT(EVOK==(rc=((AHDR2FN*)adocv.f)(~cellatoms,partition,wv,partition,0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc),rc),D) break;}  // mean for FL
 ...
 if(!((I)ai&1)){
  ...
  if((UI)maxfretsize<sizeof(localfrets)-NORMAH*SZI){frets=(A)localfrets; AT(frets)=0; AR(frets)=0; AFLAGFAUXAUDIT(frets,0)} // Cut tests the type field - only.  If debug, the flag also  rank must be valid in case we call rankex
  // we can write the frets over w if w is inplaceable, DIRECT, has items bigger than an I4, not 32-bit or atom, and not u/..~  w has always been copied to a new buffer by the sort
  else if((SZI==8) && ((((AFLAG(w)&(AFVIRTUAL|AFNJA))-((I)jtfg&JTINPLACEW))&(-(I)(AT(w)&DIRECT))&AC(w)&(4-celllen))<0) && likely(AR(w)) && (likely(w!=a)||likely(FAV(self)->id!=CSLDOTDOT)) )frets=w;
  ...
  if((UI)maxfretsize<sizeof(localfrets)-NORMAH*SZI){frets=(A)localfrets; AT(frets)=0; AR(frets)=0; AFLAGFAUXAUDIT(frets,0)} // Cut tests the type field - only; for memaudit we need flag too, and rank in case we rank2ex
  // we can write the frets over w if w is inplaceable, DIRECT, has items bigger than an I4, not 32-bit or atom, and not u/..~  w has always been copied to a new buffer by the sort
  else if((SZI==8) && ((((AFLAG(w)&(AFVIRTUAL|AFNJA))-((I)jtfg&JTINPLACEW))&(-(I)(AT(w)&DIRECT))&AC(w)&(4-celllen))<0) && likely(AR(w)) && (likely(w!=a)||likely(FAV(self)->id!=CSLDOTDOT)))frets=w;
  else GATV0(frets,LIT,maxfretsize,0);   // 1 byte per fret is adequate, since we have padding
  fretp=CUTFRETFRETS(frets);  // Place where we will store the fret-lengths.  They are 1 byte normally, or 5 bytes for groups longer than 254
...
#define keyslashvalues(w)CCM(w,CPLUS)+CCM(w,CMIN)+CCM(w,CMAX)
 ...
 default: f2=jtkey; flag |= (FAV(w)->flag&VNOLOCCHG); break;  // pass through NOLOCCHG.
 }
...
// by /. and ;.1 must be the same as far as flags etc.
 ARGCHK1(w);
 I flag; if(NOUN&AT(w)){RZ(h=fxeachv(1L,w)); flag=gflg(h)+VGERL;}else flag=(FAV(w)->flag&VNONAME+VNOSELF);  // pass through rollup flags
