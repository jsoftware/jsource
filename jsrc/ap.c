...
#define PREFIXPFX(f,Tz,Tx,pfx,vecfn,retstmt)  \
 AHDRP(f,Tz,Tx){I i;Tz v;                                 \
  if(d==1)DQ(m, *z++=v=    *x++; DQ(n-1, *z=v=pfx(v,*x); ++z; ++x;))  \
   ...
   DO(d, z[i]=    x[i];); x+=d;                                        \
   DQ(n-1, vecfn(~d,z+d,z,x,0,jt); z+=d; x+=d; ); z+=d;                     \
...
static B jtpscangt(J jt,I m,I d,I n,B*z,B*x,I apas){
 I a=apas&1, pp=PEXT0(apas,1,1), pa=PEXT0(apas,2,1), ps=apas>>3;  // extract values from mask
 if(d==1){I i;
  for(i=0;i<m;++i){
...
AHDRP(pluspfxD,D,D){I i;
 ...
 if(d==1){
  ...
  for(i=0;i<m;++i){                                              
   ...
   DQ(n-1, plusDD(~d,z+d,z,x,0,jt); z+=d; x+=d;); z+=d;
  }
 }
 R NANTEST?EVNAN:EVOK;
}   /* for associative functions only */
...
PREFIXPFX( pluspfxI2, I2, I2,  pfxplus, plusI2I2 , R EVOK; )
static OP1XYZ(plus,I,I,I4,pfxplus)
PREFIXPFX( pluspfxI4, I4, I4,  pfxplus, plusI4I4 , R EVOK; )
...
static F2(jtseg){F12IP;A z;I c,k,m,n,*u,zn;
 ARGCHK2(a,w);
...
// obsolete  wr=AR(w); r=(RANKT)jt->ranks; r=wr<r?wr:r; RESETRANK;
 ...
 if(((1-n)&-wn)>=0){R wcr?RETARG(w):reshape(apip(shape(w),zeroionei(1)),w);}  // n<2 or wn=0
 VARPS adocv; varps(adocv,self,wt,1);  // fetch info for f/\ and this type of arg
 if(!adocv.f)R IRS1(jtinfixprefix1,jt,w,wcr,self);  // if there is no special function for this type, do general scan
 ...
 if(unlikely((255&~EVNOCONV)&rc)){jsignal(rc); R (rc>=EWOV)?IRS1(jtpscan,jt,w,wcr,self):0;} else R unlikely(((adocv.cv+VRD)&VRI))&&likely(rc!=EVNOCONV)?cvz(adocv.cv,z):z;   // retry if needed; convert if needed & not suppressed
...
static DF2(jtmovfslash){F12IP;A x,z;B b;C id,*wv,*zv;I d,m,m0,p,t,wk,wt,zi,zk,zt;
 ...
 m0=rei0(a); m=REPSGN(m0); m=(m^m0)-m; m^=REPSGN(m);  // m0=infx x,  m=abs(m0), handling IMIN
