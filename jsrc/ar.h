 ...
 AHDRR(f,Tz,Tx){I i;Tz v;                              \
  if(d==1){x += m*n; z+=m; DQ(m, v=*--x; DQ(n-1, --x; v=pfx(*x,v);); *--z=v;)}  \
   ...
   for(i=0;i<m;++i,z-=d){I rc;                                    \
    Tx* RESTRICT y=x; x-=d; if(255&(rc=vecfn1(AH2ANP_v(d,x,y,z,jt)), rc=rc<0?EWOVIP+EWOVIPMULII:rc))R rc; x-=d;        \
    DQ(n-2,    if(255&(rc=vecfnn(AH2ANP_v(d,x,z,z,jt)), rc=rc<0?EWOVIP+EWOVIPMULII:rc))R rc; x-=d;);        \
  }}R EVOK;}
...
#define REDUCEPFXIDEM2(f,Tz,Tx,pfx,vecfn)  \
 AHDRR(f,Tz,Tx){I i;                              \
  if(d==1){x += m*n; z+=m; DQ(m, Tz v0=(Tz)*--x; Tz v1=v0; if(!(n&1))v1=*--x;  DQ((n-1)>>1, x-=2; v0=pfx(x[0],v0); v1=pfx(x[1],v1); ); v0=pfx(v0,v1); *--z=v0;)}  \
   ...
   for(i=0;i<m;++i,z-=d){                                    \
    Tx* RESTRICT y=x; x-=d; vecfn(AH2ANP_v(d,x,y,z,jt)); x-=d;        \
    DQ(n-2,    vecfn(AH2ANP_v(d,x,z,z,jt)); x-=d;);        \
  }}R EVOK;}
...
#define REDUCENAN(f,Tz,Tx,pfx,vecfn)  \
 AHDRR(f,Tz,Tx){I i;Tz v;                              \
  ...
  if(d==1){x += m*n; z+=m; DQ(m, v=*--x; DQ(n-1, --x; v=pfx(*x,v);); *--z=v;)}  \
   ...
   for(i=0;i<m;++i,z-=d){I rc;                                    \
    Tx* RESTRICT y=x; x-=d; if(255&(rc=vecfn(AH2ANP_v(d,x,y,z,jt)), rc=rc<0?EWOVIP+EWOVIPMULII:rc))R rc; x-=d;        \
    DQ(n-2,    if(255&(rc=vecfn(AH2ANP_v(d,x,z,z,jt)), rc=rc<0?EWOVIP+EWOVIPMULII:rc))R rc; x-=d;);        \
  }}                                                               \
