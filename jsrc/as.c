...
#include "result.h"
  ...
  else if(unlikely(a==z)){RZ(z=virtual(z,0,AR(a))); AN(z)=AN(a); MCISH(AS(z),AS(a),wcr-1);}
...
// obsolete  wr=AR(w); r=(RANKT)jt->ranks; r=wr<r?wr:r; RESETRANK;
 ...
 if(!adocv.f)R IRS1(jtssg,jt,w,wcr,self);   // if not supported atomically, go do general suffix, passing in IRS info
 ...
 if(unlikely((255&~EVNOCONV)&rc)){if(unlikely(rc==EVNOCONV))R z; jsignal(rc); R jt->jerr>=EWOV?z=IRS1(jtsscan,jt,w,wcr,self):0;} else RETF(unlikely(((adocv.cv+VRD)&VRI))&&likely(rc!=EVNOCONV)?cvz(adocv.cv,z):z);
}    /* f/\."r w main control */
