...
F1(jtbsdot){F12IP;A f;AF f1=jtsuffix,f2=jtoutfix;I flag=FAV(ds(CBSDOT))->flag&~(VASGSAFE+VFIX+VNOSELF);C id;V*v;  // init flag is IRS1
 ARGCHK1(w);
 A z; fdefallo(z)
 if(NOUN&AT(w)){
...
#define xassocvalues(w) CCM(w,CSTAR)+CCM(w,CMAX)+CCM(w,CMIN)+CCM(w,CPLUSDOT)+CCM(w,CSTARDOT)+CCM(w,CBW0000)+CCM(w,CBW0001)+CCM(w,CBW0011)+CCM(w,CBW0101)+CCM(w,CBW0111)+CCM(w,CBW1111)
 ...
 fdeffillall(z,0,CBSDOT,VERB,f1,f2,w,0L,0L,flag|(v->flag&VFIX+VNOSELF),RMAX,0L,RMAX,fffv->localuse.lu0.cachedloc=0,FAV(z)->localuse.lu1.redfn=v->id==CSLASH?v->localuse.lu1.redfn:0)
