...
#define SUFFIXPFX(f,Tz,Tx,pfx,vecfn,retstmt)  \
 AHDRS(f,Tz,Tx){I i;Tz v;                                     \
  ...
  if(d==1)DQ(m, *--z=v=    *--x; DQ(n-1, --x; --z; *z=v=pfx(*x,v);))  \
   ...
   DQ(d, *--z=    *--x;);                                        \
   DQ(n-1, Tz *y=z; z-=d; x-=d; vecfn(~d,z,x,y,0,jt););                     \
 }}retstmt}
...
AHDRS(plussfxD,D,D){I i;
 ...
 if(d==1){
  ...
  for(i=0;i<m;++i){
   DQ(d, *--z=    *--x;);
   DQ(n-1, D *y=z; z-=d; x-=d; plusDD(~d,z,x,y,0,jt););
  }
 }
 R NANTEST?EVNAN:EVOK;
}
...
SUFFIXPFX( plussfxI2, I2, I2, pfxplus, plusI2I2, R EVOK; )
SUFFIXPFX( plussfxI4, I4, I4, pfxplus, plusI4I4, R EVOK; )
...
// obsolete  wr=AR(w); r=(RANKT)jt->ranks; r=wr<r?wr:r; f=wr-r; RESETRANK;
 ...
 if(((n-2)|(wn-1))<0){if(FAV(FAV(y)->fgh[0])->flag&VISATOMIC2){R wcr?RETARG(w):reshape(apip(shape(w),zeroionei(1)),w);}else R IRS1(jtsuffix,jt,w,wcr,self);}  // if empty arg, or just 1 cell in selected axis, convert to f/\ which handles the short arg 

   // note that the above line always takes the r==0 case
 VARPS adocv; varps(adocv,self,wt,2);  // analyze f - get suffix routine
 if(!adocv.f)R IRS1(jtssg,jtfg,w,wcr,self);   // if not supported atomically, go do general suffix
 ...
 if(unlikely((255&~EVNOCONV)&rc)){if(unlikely(rc==EVNOCONV))R z; jsignal(rc); R jt->jerr>=EWOV?IRS1(jtsscan,jt,w,wcr,self):0;} else RETF(unlikely(((adocv.cv+VRD)&VRI))&&likely(rc!=EVNOCONV)?cvz(adocv.cv,z):z);
}    /* f/\."r w main control */


static F2(jtomask){F12IP;A c,r,x,y;I m,n,p;
 ARGCHK2(a,w);
 m=rei0(a); p=ABSOFLO(m,IMAX); SETIC(w,n);
...
static DF2(jtofxinv){F12IP;A f,fs,z;C c;I t;V*v;
 ...
 A z1; z=irs2(z0,dfv2(z1,a,w,bslash(fs)),c==CPLUS?ds(CMINUS):f, RMAX,-1L,jtatomic2);  // x u/\. y = (u/ y) uinv"(_ _1) x u/\ y   this rank is 0 for atomics; causes loop migration
...
static DF2(jtofxassoc){F12IP;A f,i,j,p,s,x,z;C id,*zv;I c,d,k,kc,m,r,t;V*v;VA2 adocv;
 ...
 SETIC(w,m); k=rei0(a); c=ABSOFLO(k,IMAX);  // m = # items in w; k is value of a; c is # items per suffix
