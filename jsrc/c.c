...
static DF1(ad1){DECLFG;A z;UC od=jt->cx.cx_c.db; 
 RZ(w); 
 jt->cx.cx_c.db=0; z=CALL1(f1,  w,fs); jt->cx.cx_c.db=od;
 if(EVTHROW==jt->jerr)R 0;
 RESETERR;  
 R z?z:AT(gs)&NOUN?gs:CALL1(g1,  w,gs);
}

static DF2(ad2){DECLFG;A z;UC od=jt->cx.cx_c.db;
 RZ(a&&w); 
 jt->cx.cx_c.db=0; z=CALL2(f2,a,w,fs); jt->cx.cx_c.db=od;
 if(EVTHROW==jt->jerr)R 0;
 RESETERR; 
