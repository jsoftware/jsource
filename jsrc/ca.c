...
#define SPECAND (IDBIT(CCOMMA)|IDBIT(CBOX)|IDBIT(CGRADE)|IDBIT(CSLASH)|IDBIT(CPOUND)|IDBIT(CCEIL)|IDBIT(CFLOOR)|IDBIT(CRAZE))  // mask for all special cases
  ...
  if((-AN(na)&-AR(na))<0){  // noun is not atomic and not empty
   ...
   }else if(unlikely((((I)FAV(w)->valencefns[0]^(UI)jtwords)|visa)==0)){RZ(a=fsmvfya(a)); f1=jtfsmfx; vbf2=0; flag&=~(VIRS1);   // m&;:
   }else if(unlikely((((I)FAV(w)->valencefns[0]^(I)jtcrc1)|visa)==0)){RZ(h=crccompile(a)); f1=jtcrcfixedleft; vbf2=0; flag&=~(VIRS1); // m&128!:3
   }
