...
#define ZZFLAGWORD state
 ...
 if(FAV(self)->id==CCUT){   // see if we are acting on behalf of /.[.]  Fall through if not
  ...
  if(!(AT(a)&MARK)){  // dyadic forms
   ...
   if((((wt&(B01|LIT|INT|FL|C2T|C4T))-1)|(k-1)|(((BW==32&&wt&FL&&k==SZD)-1)&((k&-k&(2*SZI-1))-k)))>=0){a=w; ak=k; at=(wt+B01)&~B01;  // monadic forms: if w is an immediate type we can handle, and the length is a machine-word length, use w unchanged
...
#define BSIZE 32  // # bytes in a block
   __m256i bitendmask=_mm256_loadu_si256((__m256i*)(validitymask+PEXT0(-n,LGSZI,NPAR-1)));  // since alignment never changes, we can predict the validity for the last block
 ...
 for(hi=rs[1];hi;--hi){C *svv, *dvv;  // pointers within the lower-right corner, starting at the top-left of the corner.  Before that we use them to advance through top-right
  ...
  for(vi=rs[0];vi;--vi){A opcell=virtw;
   ...
   if(state&(STATEREFLECTX|STATEREFLECTY|STATETAKE)){  // something might be truncated/reflected
    ...
    if(state&STATEREFLECTX){A t; I tr=AR(opcell)-((UI)AR(opcell)>0); RZ(opcell=t=IRS1(jtreverse,jt,opcell,tr,0L));}  // reverse horizontal
   }
...
// by /. and ;.1 must be the same as far as flags etc.  For the shared case, inplacing is OK
 ...
 k=rei0(w); if(!(AT(w)&B01))w=sc(k);  // force w to INT/B01
 A z; fdefallo(z)
