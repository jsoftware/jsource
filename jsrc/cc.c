...
#define CUTSWITCH(EACHC)  \
 switch(id){A z,*za;C id1,*v1,*zc;I d,i,j,ke,q,*zi,*zs;                 \
  ...
  case CRIGHT:                                                               \
   ...
   GA(z,t,d,id==CCOMMA?2:1+r,s-1); zc=CAV(z); fillv0(t), mvc(d<<bplg(t),zc,sizeof(jt->fillv0),jt->fillv0);                 \
...
#define ZZFLAGWORD state
 ...
 case CHEAD: ;
  ...
  EACHCUT(if(d)MC(zc,id==CHEAD?v1:v1+k*(d-1),k); else mvc(k,zc,sizeof(jt->fillv0),jt->fillv0); zc+=k;);
  break;
 case CSLASH: ;
