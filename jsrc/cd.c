...
static I jtispoly(J jt,A w){A e,f,g,h,x,y;B nf,ng,vf,vg;C c,id;I k,m,n,t;V*v;
 ...
 if(nf&&1>=AR(x)&&c==CPOLY){
  ...
  RZ(equ(e,floor1(e))&&all1(le(num[0],e)));
...
static F1(jtfpolyc){A b;B*bv;I m,n;
 RZ(b=ne(w,num[0])); bv=BAV(b);
 m=n=AN(w); DO(n, if(bv[--m])break;); ++m;
 if(m<n)RZ(w=take(sc(m),w)); n=m;
 switch(n){
  case 1: R qq(head(w),num[0]);
  case 3: if(equ(w,over(v2(0L,0L),num[1])))R ds(CSTARCO); break;
  case 2: 
   ...
   if(equ(w,over(num[0],scf((D)0.5))))R ds(CHALVE);
   if(equ(w,over(num[0],scf(PI    ))))R ds(CCIRCLE);
...
static F2(jtfplus){
 RZ(a&&w);
 if(iscons(a)&&equ(VAV(a)->fgh[0],num[0]))R w;
 if(iscons(w)&&equ(VAV(w)->fgh[0],num[0]))R a;
 R folk(a,ds(CPLUS),w);
}

static F2(jtfminus){
 RZ(a&&w);
 if(iscons(a)&&equ(VAV(a)->fgh[0],num[0]))R fnegate(w);
 if(iscons(w)&&equ(VAV(w)->fgh[0],num[0]))R a;
...
static F2(jtftymes){A x,y;B b,c;I k;
 ...
 if(b&&CFORK==ID(w)&&iscons(y))R ftymes(qq(tymes(x,VAV(y)->fgh[0]),num[0]),VAV(w)->fgh[2]);
...
static F1(jtdpoly){A c,e,x;I n,t;
 ...
 if(!n||t&NUMERIC)R 2>=n?qq(2==n?tail(w):cvt(n?t:B01,num[0]),num[0]):fpolyc(behead(tymes(w,IX(n))));
 x=AAV0(w);
 if(1<n||1>=AR(x))R dpoly(poly1(w));
 ASSERT(2==AR(x)&&2==*(1+AS(x)),EVDOMAIN);
 c=irs1(x,0L,1L,jthead);
 e=irs1(x,0L,1L,jttail);
 R amp(box(stitch(tymes(c,e),minus(e,num[1]))),ds(CPOLY));
}
...
static F1(jtipoly){A b,c,e,p=0,q=0,x;I n,t;
 ...
 RZ(e=plus(num[1],irs1(x,0L,1L,jttail)));
 RZ(b=ne(e,num[0]));
...
static F1(jtdiffamp0){A f,g,h,x,y;B nf,ng;C id;V*v;
 ...
 switch(id){
  case CPLUS:  R qq(num[1],num[0]);
  case CSTAR:  R qq(x,num[0]);
  case CMINUS: R qq(num[nf?-1:1],num[0]);
  case CDIV:   R nf?eva(x,"(-x)&%@*:"):qq(recip(x),num[0]);
  ...
  case CEXP:
   ...
   if(INT&AT(y))switch(*AV(y)){
    case 0:    R qq(num[0],num[0]);
    case 1:    R qq(num[1],num[0]);
    case 2:    R ds(CPLUSCO);
   }
   R eva(x,"x&*@(^&(x-1))");
  case CCIRCLE:
...
static F1(jtdiff0){A df,dg,dh,f,g,h,x,y,z;B b,nf,ng,vf,vg;C id;I m,p,q;V*v;
 ...
 switch(id){
  ...
  case CGE:
  case CLEFT:
  case CRIGHT:    R qq(num[1],num[0]);
  case CPLUSCO:   R qq(num[2],num[0]);
  case CNOT:
  case CMINUS:    R qq(num[-1],num[0]);
  case CFCONS:    R qq(num[0],num[0]);
  case CSTARCO:   R ds(CPLUSCO);
  case CHALVE:    R qq(connum(3L,"1r2"),num[0]);
  case CCIRCLE:   R qq(pie,num[0]);
  case CDIV:      R eval("- @%@*:");
  case CSQRT:     R eval("-:@%@%:");
  case CEXP:      R w;
  case CLOG:      R ds(CDIV);
  case CJDOT:     R qq(a0j1,num[0]);
  case CRDOT:     R atop(ds(CJDOT),w);
  case CDDOT:     if(vf&&ng)R ddot(f,increm(g)); break;
  case CPOWOP:    
   if(vf&&ng&&!AR(g))switch(p=i0(g)){
    case -1: R diff0(inv(f));
    case  0: RE(0); R qq(num[1],num[0]);
    case  1: R diff0(f);
    default:
   ...
   break;
  case CQQ:
   if(!AR(f)&&NUMERIC&AT(f)&&ng&&equ(g,num[0]))R qq(num[0],num[0]);
  ...
  case CTILDE:
   if(vf)switch(ID(f)){
    case CPLUS:   R qq(num[2],num[0]);
    case CSTAR:   R ds(CPLUSCO);
    case CMINUS:
    case CLOG:
    case CDIV:    R qq(num[0],num[0]);
    case CEXP:    R eva(w,"x * >:@^.");
   }
   break;
  case CFORK:
   h=v->fgh[2];
   if(NOUN&AT(f))R diff0(folk(qq(f,num[0]),g,h));
   ...
   switch(ID(g)){
    ...
    case CEXP:    if(1==q){A k;
                   RZ(k=df1(num[0],h));
                   if(equ(k,num[0]))R qq(num[0],num[0]);
                   if(equ(k,num[1]))R df;
                   if(equ(k,num[2]))R ftymes(df,ftymes(h,f));
                   R ftymes(df,ftymes(h,folk(f,g,qq(decrem(k),num[0]))));
                  }
 }}
 R A0;
}
...
static F1(jtintgamp0){A f,g,h,x,y;B nf,ng;C id;V*v;
 ...
 switch(id){
  case CPLUS:  R ipoly(over(x,num[1]));
  case CSTAR:  R ipoly(over(num[0],x));
  case CMINUS: R nf?ipoly(over(x,num[-1])):ipoly(over(negate(x),num[1]));
...
static F2(jtintgatop){A df,f=a,g=w,q,x,y;I m,n;V*v;
 ...
 if(2==m){
  RZ(q=v2(0L,1L));
  RZ(x=df1(q,tdot(f)));
  RZ(y=equ(num[1], tail(x))?intg0(g):atop(fpolyc(tymes(q,x)),intg0(g)));
  R    equ(num[0],head(x))?y       :folk(y,ds(CPLUS),amp(head(x),ds(CSTAR)));
 }
 if(1==n||2==n){
  df=atop(intg0(f),g); 
  if(1==n)R df;
  RZ(x=df1(num[1],tdot(g)));
  R equ(x,num[1])?df:atop(amp(ds(CDIV),x),df);
 }
 v=VAV(g);
...
static F1(jtintg0){A df,dh,f,g,h;B nf,ng,vf,vg;C id;I m,n,p,q;V*fv,*gv,*v;
 ...
 switch(id){
  ...
  case CSTARCO:   R ipoly(over(v2(0L,0L),num[1]));
  ...
  case CPOWOP:    
   ...
   break;
  case CQQ:
   if(!AR(f)&&NUMERIC&AT(f)&&ng&&equ(g,num[0]))R amp(f,ds(CSTAR));
  ...
  case CTILDE:
   if(vf)switch(ID(f)){
    ...
    case CSTAR:   R ipoly(over(v2(0L,0L),num[1]));
  ...
  case CFORK:
   h=v->fgh[2]; 
   if(NOUN&AT(f))R intg0(folk(qq(f,num[0]),g,h));
...
F2(jtddot){A x,*xv,y,z;AF f;I j,n,p,q,r,*wv;
 ...
 if(!r){
  if(!p){V*v=VAV(a); R v->mr||v->lr||v->rr?qq(a,num[0]):a;}
...
static F1(jtdiff){A df,dh,f,g,h,z;B nf,ng,vf,vg;C id;I r;V*v;
 ...
 if(z=diff0(w))R id==CQQ&&ng&&equ(g,num[0])?z:icube(z);
