...
A jtfolk(J jtfg,A f,A g,A h){F12IP;A p,q,x,y;AF f1=0,f2=0;B b;C c,fi,gi,hi;I flag,flag2=0,j,m=-1,fline,hcol;V*fv,*gv,*hv,*v;
 ...
 // Start flags with NOLOCCHG (if g and h are safe), and with INPLACEOK to match the setting of f1,f2.  Turn off inplacing that neither f nor h can handle
 if(NOUN&AT(f)){  /* nvv, including y {~ x i. ] */
  // f will be INCORPed by fdef
  flag=((gv->flag&hv->flag)&VNOLOCCHG+VNONAME+VNOSELF);  // We accumulate the flags for the derived verb.  Start with NOLOCCHG if all descendants are.
  ...
  if(fi!=CCAP){
   // vvv fork.  inplace if f or h can handle it, NOLOCCHG only if all 3 verbs can
   flag=((fv->flag&gv->flag&hv->flag)&VNOLOCCHG+VNONAME+VNOSELF);  // We accumulate the flags for the derived verb.  Start with NOLOCCHG if all descendants are.
   ...
   // capped fork  inplace if h can handle it, NOLOCCHG if gh are safe
   flag=((gv->flag&hv->flag)&VNOLOCCHG+VNONAME+VNOSELF);  // We accumulate the flags for the derived verb.  Start with NOLOCCHG if all descendants are, and inplacing if h handles it
 ...
 // special code for x ((<[!.0] |) * ]) y, implemented as if !.0, also if <:
  ...
  if(unlikely(fi==CLEFT)){
   I d=gv->id; d=d==CFIT&&gv->localuse.lu1.cct==1.0?FAV(gv->fgh[0])->id:d;  // middle -. of [-.-. .  We implement as if !.0 given on g, so we allow the user to give that
 ...
 if(likely(!f1))f1=fork1tbl[SHMSK(0x200110,fline<<2,3)][SHMSK(0b00110,hcol,1)];  // fline: 0/3/4->0,  1/2->1, 5->2   hcol: / 0/3/4->0,  1/2->1
 if(likely(!f2)){
...
// obsolete  ={{0,(t)&TRAVERSIBLE,0,(t),ACPERMANENT,0,(ar)},{{.valencefns={f0,f1},.fgh={f,g,h},.localuse={initpm1,initpm2},.flag=(vflg),.flag2=(vflg2),.lrr=(RANK2T)((rl<<RANKTX)+rr),.mr=(RANKT)rm,.id=idd,.lu2.lc=lcc}}}
// obsolete #define PRIMALL(idda,idd,t,f0,f1,f,g,h,initpm1,initpm2,rm,rl,rr,vflg,vflg2,an,ar,lcc) [idda]PRIMASSIGN(idda,idd,t,f0,f1,f,g,h,initpm1,initpm2,rm,rl,rr,vflg,vflg2,an,ar,lcc)
// obsolete #define PRIMATOMIC2(vaid,id,t,f0,f1,rm,rl,rr,vflg,vflg2) PRIMALL(vaid,id,t,f0,f1,0,0,0,.lu1.uavandx=0,VA2##vaid*sizeof(VA),rm,rl,rr,VISATOMIC2|(vflg),vflg2,0,0,
// obsolete  VA2##vaid+0x80*(vaid==CLT||id==CGT||id==CLE||id==CGE||id==CEQ||id==CNE))
...
A jthook(J jt,A a,A w,A h){AF f1=0,f2=0;C c,d,e,id;I flag=VFLAGNONE,linktype=0;V*u,*v;
 ...
 if(likely(h==0)){
  ...
  I flag2=VF2NAMELESS, ffix=VNONAME+VNOSELF; A ta=a; ta=AT(a)&NOUN?ds(CPLUS):ta; flag2&=(FAV(ta)->flag2<<(VF2NAMELESSX-VF2PRIMX)); ffix&=FAV(ta)->flag;  // CPLUS to make nouns look primitive
  ...
  if(unlikely(t==0)){
   ...
   // list of the parts of speech of the unexecutable fragment
   jsignal(EVSYNTAX);
   GAT0(z,INT,2,1); IAV1(z)[0]=rtnx&3; IAV1(z)[1]=PEXT0(rtnx,2,3); IAV1(z)[2]=rtnx>>4;  // install parts of speech in order
   jteformat(jt,ds(CENQUEUE),z,zeroionei(1),0); R0;
  }
  ...
  flag=VNONAME+VNOSELF;  // we will set VNONAME+VNOSELF if all branches are set
