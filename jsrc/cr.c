...
#include "result.h"
  ...
  GA00(zz,AT(z),0L,wf+AR(z)); zzs=AS(zz); MCISH(zzs,AS(w),wf); MCISH(zzs+wf,AS(z),AR(z));
 }
...
#include "result.h"
  ...
  GA00(zz,AT(z),0L,AR(w)+AR(z)); zzs=AS(zz); MCISH(zzs,AS(w),AR(w)); MCISH(zzs+AR(w),AS(z),AR(z));
 }
...
#else  // obsolete
A jtrank2ex(J jt,AD * RESTRICT a,AD * RESTRICT w,A fs,UI lrrr,UI lcrrcr,AF f2){  // scaf must combine 
#endif
...
#include "result.h"
  ...
  GA00(zz,AT(z),0L,lof+lif+AR(z)); zzs=AS(zz);
...
#include "result.h"
  ...
  GA00(zz,AT(z),0L,ar+AR(z)); zzs=AS(zz); MCISH(zzs,as,ar); MCISH(zzs+ar,AS(z),AR(z));  // allocate result, copy frame and shape
 }
...
F2(jtqq){F2PREFIP;AF f1,f2;I hv[3],n,r[3],vf,flag2=0,*v;A ger=0;C lc=0;
 ...
 if(unlikely(NOUN&AT(a))){  // could be gerund"n or noun"n
  ...
  // give a warning for }"n
  if(unlikely(av->id==CAMEND&&JT(jt,deprecct)!=0))RZ(jtdeprecmsg(jt,4,"(004) m}\"n should be executed without \" using leading axes of a: in m\n"));
