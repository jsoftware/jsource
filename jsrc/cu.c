...
static DF2(jtunder2){F2PREFIP;DECLFG;A fullf; RZ(fullf=atop(invrecur(fix(gs,zeroionei[0])),amp(fs,gs))); R (FAV(fullf)->valencefns[1])(FAV(fullf)->flag&VJTFLGOK2?jtinplace:jt,a,w,fullf);}
...
F2(jtunder){A x;AF f1,f2;B b,b1;C c,uid;I m,r;V*u,*v;
 ...
 switch(v->id){
  ...
  case CAMP:  
   ...
   b=CBDOT==uid&&(x=u->fgh[1],!AR(x)&&INT&AT(x)&&(m=*AV(x),(UI)(m-16)<=(UI)(32-16)));   // b if f=m b. where m is atomic int 16<=m<=32
