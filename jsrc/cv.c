...
static A jtfitct(J jt,A a,A w,I cno,A z){
 ...
 fdeffillall(z,0,CFIT,VERB,cno==5?(AF)(jtfitfill12):(AF)(jtfitct1),aff2[cno&~4],a,w ,0L,FAV(a)->flag&(VIRS1|VIRS2|VISATOMIC1|VFCOMPCOMP|VASGSAFE),(I)(FAV(a)->mr),lrv(FAV(a)),rrv(FAV(a)),fffv->localuse.lu0.cachedloc=0,FAV(z)->localuse.lu1.cct = 1.0-d);  // preserve INPLACE flags
 R z;
}
...
//  ASSERTVN(a,w);  // a must be a verb, w a noun (except for $!.v)
 ...
 if(likely(!(AT(w)&VERB))){  // is v a noun?
  ...
  case CQQ: ;
   ...
   fdeffillall(z,0L,CFIT,VERB,jtvalenceerr,jtsumattymes1,a,w,0L,FAV(a)->flag&(VIRS2|VFCOMPCOMP|VASGSAFE),m,l,r,fffv->localuse.lu0.cachedloc=0,FAV(z)->localuse.lu1.fittype=wval) RETF(z);  // supports IRS
  case CSLASH: ;
...
fillreshape:;
   ...
   fdeffill(z,0L,CFIT,VERB,jtfitfill12,jtfitfill12,a,w,0L,FAV(a)->flag&(VIRS1|VIRS2|VASGSAFE),m,l,r) RETF(z);  // various allowing empty fill
  ...
  if(rtn1==jtreshape||rtn1==jtreitem)
   fdeffill(z,0L,CFIT,VERB,jtvalenceerr,jtreshapeblankfn,a,w,0L,FAV(a)->flag&(VIRS1|VIRS2|VASGSAFE),m,l,r) FAV(z)->localuse.lu1.fittype=rtn1==jtreitem; RETF(z);  // fittype tells whether ($,) or $
 }
 ASSERT(0,EVDOMAIN);
} 
