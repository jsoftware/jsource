...
static A jtfitct(J jt,A a,A w,I cno,A z){
 ...
 fdeffillall(z,0,CFIT,VERB,cno==5?(AF)(jtfitfill12):(AF)(jtfitct1),aff2[cno&~4],a,w ,0L,FAV(a)->flag&(VIRS1|VIRS2|VISATOMIC1|VFCOMPCOMP|VNOLOCCHG|VNONAME+VNOSELF),(I)(FAV(a)->mr),lrv(FAV(a)),rrv(FAV(a)),fffv->localuse.lu0.cachedloc=0,FAV(z)->localuse.lu1.cct = 1.0-d);  // preserve INPLACE flags
 R z;
}
...
//  ASSERTVN(a,w);  // a must be a verb, w a noun (except for $!.v)
 ...
 if(likely(!(AT(w)&VERB))){  // is v a noun?
  ...
  case CEXP:
   ...
   fdeffill(z,0L,CFIT,VERB,jtvalenceerr,jtfitexp2,a,w,0L,VFLAGNONE+VNOLOCCHG+VNONAME+VNOSELF,m,l,r) RETF(z);  // ^!.f
  case CPOLY:
   ASSERT(AT(w)&NUMERIC,EVDOMAIN);
   fdeffill(z,0L,CFIT,VERB,jtvalenceerr,jtfitpoly2,a,w,0L,VFLAGNONE+VNOLOCCHG+VNONAME+VNOSELF,m,l,r) RETF(z);  // p.!.f
  ...
  case CPOWOP:  // support for #^:_1!.n
   ...
   if(c==CPOUND){ASSERT(!AR(w),EVRANK); fdeffill(z,0L,CFIT,VERB,jtvalenceerr,jtfitfill12,a,w,0L,VFLAGNONE+VNOLOCCHG+VNONAME+VNOSELF,m,l,r) RETF(z);}  // #^:_1!.f
