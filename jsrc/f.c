...
struct fmtbuf fmtlong(struct fmtbuf fb, E v){
 ...
 for(i=0;i<2;++i){
  ...
  while((currbit>=0||i==1)&&nextexp>=0){  // carry on till integer part finished.  Break at end of valid bits, but only if there are no more coming later
   ...
   I cry=nextexp>currexp?0:(I)SHMSK8(bits,currbit,1);  // carry-in to lowest position: the bit we are working on (0 if a skipped exp)
  ...
  while(currbit>=0){
   ...
   if(nextexp==currexp&&(I)SHMSK8(bits,currbit,1)){  // non-skipped 1 bit
