...
// INITJT(jjt,int64rflag)=0;
 ...
 INITJT(jjt,implocref)[0] = fdef(0,CTILDE,VERB,jtvalenceerr1,jtvalenceerr2, uimp,0L,0L, 0, RMAX,RMAX,RMAX); AC(INITJT(jjt,implocref)[0])=ACUC1;  //create 'u.'~, mark an not abandoned (no ra() needed)
 A vimp=ca(mnuvxynam[3]); NAV(vimp)->flag|=NMIMPLOC;
 INITJT(jjt,implocref)[1] = fdef(0,CTILDE,VERB,jtvalenceerr1,jtvalenceerr2, vimp,0L,0L, 0, RMAX,RMAX,RMAX); AC(INITJT(jjt,implocref)[1])=ACUC1;  //create 'v.'~
