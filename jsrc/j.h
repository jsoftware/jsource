...
// empty (most commonly after a final local assignment). To reduce
// tpop overhead, we allow a few blocks to stay unfreed before we force a tpop.  BUT if a public value or a global assigned from a private value is not tpopped it is ineligible for inplacing, so we pop all B blocks.
#define TPOPSLACKB 0  // number of stacked blocks to allow before we tpop them (in jtxdefn) - 0 means tpop unless stack empty - for B blocks
#define TPOPSLACKT 2  // number of stacked blocks to allow before we tpop them (in jtxdefn) - 0 means tpop unless stack empty - for T blocks
...
#define CNULLNOERR(x) ({A res=(x); if(res&&(AT(res)&BOX+PYX)==BOX+PYX)res=jtpyxvalid(jt,res); res; })   // if x is 0, keep it 0; ignore error, don't resolve pyx but use it if resolved already
...
#define DF2RANK(l,r,f,self)  {if(likely((I)((l-AR(a))|(r-AR(w)))<0))R f(jt,a,w,(A)self); if((l|r)==0)R rank2ex0(a,w,(A)self,f);else{I lr=MIN((I)l,AR(a)); I rr=MIN((I)r,AR(w)); R rank2ex(a,w,(A)self,lr,rr,lr,rr,f);}}  // If there is more than one cell, run rank2ex on them.  l,r=dyad ranks, f=function to call for dyad cell
...
// bit0=h is ][
// bit4=f is ][ bit5=NVV/@:/&: bit6=f not executed on x/y (maybe on result of g, for @:)
...
#define GATVE(name,type,atoms,rank,shaape,erraction) GATVS(name,type,atoms,rank,shaape,type##SIZE,GACOPYSHAPE,erraction)  // shape must not be 0
