...
#define createcycliciterator(x)     jtcreatecycliciterator(jt,(x))
...
// obsolete #define dcapco(x,y)                 jtdcapco(jt,(x),(y))
#define dcase(x,y)                  jtdcase(jt,(x),(y))
#define dcube(x,y)                  jtdcube(jt,(x),(y))
// obsolete #define ddot(x,y)                   jtddot(jt,(x),(y))   
...
#define fauxINT(z,v,n,r) {if(AKXR(r)+(n)*SZI<=sizeof(v)){z=(A)(v); AK(z)=AKXR(r); AFLAG(z)=0/*AFNJA*/; AT(z)=INT; AC(z)=ACUC1; AN(z)=(n); AR(z)=(RANKT)(r); if(r==1)AS(z)[0]=(n);}else{GATV0(z,INT,(n),(r));}}
