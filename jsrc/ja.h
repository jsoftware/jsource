...
#define df1ip(r,x,y)                  (r=((r=(y))?(FAV(r)->valencefns[0])((J)(((I)jtfg&~JTXDEFMODIFIER)+(AT(r)&(ADV|CONJ)?JTXDEFMODIFIER:0)),(x),r,r):r))
#define df2ip(r,x,y,z)                (r=((r=(z))?(FAV(r)->valencefns[1])((J)(((I)jtfg&~JTXDEFMODIFIER)+(AT(r)&(ADV|CONJ)?JTXDEFMODIFIER:0)),(x),(y),r):r))
...
#define enframe(x)                  jtenframe(jtfg,(x))
...
#define fmfill(x0,x1,x2,x3,x4,x5,x6,x7) jtfmfill(jtfg,(x0),(x1),(x2),(x3),(x4),(x5),(x6),(x7))
...
#define grd1sp(x)                   jtgrd1sp(jtfg,(x))
#define grd1spdd(x,y,z)             jtgrd1spdd(jtfg,(x),(y),(z))
#define grd1spds(x,y,z)             jtgrd1spds(jtfg,(x),(y),(z))
#define grd1spsd(x,y,z)             jtgrd1spsd(jtfg,(x),(y),(z))
#define grd1spss(x,y,z)             jtgrd1spss(jtfg,(x),(y),(z))
#define grd1spz(x,y,z)              jtgrd1spz(jt,(x),(y),(z))
#define grd2sp(x,y)                 jtgrd2sp(jtfg,(x),(y))
#define grd2spsd(x,y,z)             jtgrd2spsd(jtfg,(x),(y),(z))
#define grd2spss(x,y,z)             jtgrd2spss(jtfg,(x),(y),(z))
#define gri(x0,x1,x2,x3,x4)         jtgri(jtfg,(x0),(x1),(x2),(x3),(x4))
#define gri1(x0,x1,x2,x3,x4)        jtgri1(jtfg,(x0),(x1),(x2),(x3),(x4))
#define gru(x0,x1,x2,x3,x4)         jtgru(jt,(x0),(x1),(x2),(x3),(x4))
#define gru1(x0,x1,x2,x3,x4)        jtgru1(jtfg,(x0),(x1),(x2),(x3),(x4))
#define grs(x0,x1,x2,x3,x4)         jtgrs(jt,(x0),(x1),(x2),(x3),(x4))
#define grx(x0,x1,x2,x3,x4)         jtgrx(jtfg,(x0),(x1),(x2),(x3),(x4))
...
#define irs1ip(x0,x1,x2,x3)         jtirs1(jtfg,(x0),(x1),(x2),(x3))
#define irs2ip(x0,x1,x2,x3,x4,x5)   jtirs2(jtfg,(x0),(x1),(x2),(x3),(x4),(x5))
...
#define lbox(x)                     jtlbox(jtfg,(x),ltext)
#define lchar(x)                    jtlchar(jtfg,(x),ltext)
#define lcm(x,y)                    jtatomic2(jt,(x),(y),ds(CSTARDOT))    
#define lcolon(x)                   jtlcolon(jtfg,(x),ltext) 
...
#define linsert(x,y)                jtlinsert(jtfg,(x),(y),ltext) 
#define lnoun(x)                    jtlnoun(jtfg,(x),ltext)
#define lnoun0(x)                   jtlnoun0(jtfg,(x),ltext)
#define lnum(x)                     jtlnum(jtfg,(x),ltext)
#define lnum1(x)                    jtlnum1(jtfg,(x),ltext)
...
#define lrr(x)                      jtlrr(jtfg,(x),0L,ltext)
#define lsh(x)                      jtlsh(jtfg,(x),ltext)
#define lshape(x)                   jtlshape(jtfg,(x),ltext)
#define lsparse(x)                  jtlsparse(jtfg,(x),ltext)
#define lsub(x,y,z)                 jtlsub(jt,(x),(y),(z))
#define lsymb(x,y)                  jtlsymb(jtfg,(x),(y),ltext)
...
#define radescendsv(x,sv)         {scaft(x) I tt=AT(x); {if(unlikely(((tt^AFLAG(x))&TRAVERSIBLE)!=0)){AFLAGORLOCAL((x),(tt)&RECURSIBLE); sv=jtra((x),(tt),sv);}}}  // keep sv in a register
...
#define ramkrecursv(x)              {scaft(x) if(withprob(((AT(x)^AFLAG(x))&RECURSIBLE),0.1)){AFLAGORLOCAL((x),AT(x)&RECURSIBLE); x=jtra(x,AT(x),x);}}  // if block is not recursive, it must be local
...
#define ra00(x,tt)                {scaft(x) if(unlikely(((tt)&TRAVERSIBLE)!=0)){AFLAGORLOCAL((x),(tt)&RECURSIBLE); x=jtra((x),(tt),x);}}
...
#define ra0(x)                      {scaft(x) if(unlikely((AFLAG(x)&AFVIRTUAL)!=0)){RZ((x)=realize(x));}radescend(x)}
...
#define raposlocal(x,sv)   {scaft(x) I c=AC(x); if(likely(!ACISPERM(c))){if(c==1)AC(x)=ACUC2;else __atomic_fetch_add(&AC(x),1,__ATOMIC_ACQ_REL); if(unlikely(ISSPARSE(AT(x))))sv=jtra((x),SPARSE,sv);}}  // better a misbranch than an atomic instruction if c<0
#define raposacv(x)   {scaft(x) I c=AC(x); if(likely(!ACISPERM(c))){__atomic_fetch_add(&AC(x),1,__ATOMIC_ACQ_REL);}}  // ACV is guaranteed recursive
#define raposgblqcgsv(x,qct,sv) {scaft(x) I c=AC(x); if(likely(!ACISPERM(c))){__atomic_fetch_add(&AC(x),1,__ATOMIC_ACQ_REL); if(unlikely(qct==VALTYPESPARSE))sv=jtra((x),SPARSE,sv);}}  // must be recursive usecount but may be sparse
#define raposlocalqcgsv(x,qct,sv) {scaft(x) I c=AC(x); if(likely(!ACISPERM(c))){if(c==1)AC(x)=ACUC2;else __atomic_fetch_add(&AC(x),1,__ATOMIC_ACQ_REL); if(unlikely(qct==VALTYPESPARSE))sv=jtra((x),SPARSE,sv);}}  // must be recursive usecount but may be sparse
#define rarecur(x)   {scaft(x) I c=AC(x); if(likely(!ACISPERM(c))){if(c<0)AC(x)=(I)((UI)c+(ACINPLACE+ACUC1));else __atomic_fetch_add(&AC(x),1,__ATOMIC_ACQ_REL); I tt=AT(x); if(unlikely(ISSPARSE(tt)))x=jtra((x),(tt),x);}}  // ra but recursibles know to be recursive
...
#define raczapcommon(x,cond,zapfn)   {scaft(x) I c=AC(x); if(likely(!ACISPERM(c))){if(likely(cond)){zapfn AC(x)=c&=~ACINPLACE;}else{if(c<0)AC(x)=(I)((UI)c+(ACINPLACE+ACUC1));else __atomic_fetch_add(&AC(x),1,__ATOMIC_ACQ_REL);} \
                                    radescend(x)}}
...
#define rank2exip(x0,x1,x2,x3,x4,x5,x6,x7)  jtrank2ex(jtfg,(x0),(x1),(x2),REX2R((x3),(x4),(x5),(x6)),(x7))
...
#define redg(x,y)                   jtredg(jtfg,(x),(y))
...
#define repbdx(x0,x1,x2,x3)         jtrepbdx(jtfg,(x0),(x1),(x2),(x3))
...
#define sortb(x0,x1,x2)             jtsortb(jtfg,(x0),(x1),(x2))
#define sortb2(x0,x1,x2)            jtsortb2(jtfg,(x0),(x1),(x2))
#define sortb4(x0,x1,x2)            jtsortb4(jtfg,(x0),(x1),(x2))
#define sortc(x0,x1,x2)             jtsortc(jtfg,(x0),(x1),(x2))
#define sortc2(x0,x1,x2)            jtsortc2(jtfg,(x0),(x1),(x2))
#define sortu(x0,x1,x2)             jtsortu(jtfg,(x0),(x1),(x2))
#define sortu1(x0,x1,x2)            jtsortu1(jtfg,(x0),(x1),(x2))
#define sortd(x0,x1,x2)             jtsortd(jtfg,(x0),(x1),(x2))
#define sorti(x0,x1,x2)             jtsorti(jtfg,(x0),(x1),(x2))
#define sorti1(x0,x1,x2)            jtsorti1(jtfg,(x0),(x1),(x2))
