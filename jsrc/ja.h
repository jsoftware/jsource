...
#define parsex(x0,x1,x2,x3,x4)      jtparsex(jt,(x0),(x1),(x2),(x3),(x4))
...
#if MEMAUDIT&2
#define ra(x)                       {if(x){I* Zcc=&AC(x); I c=*Zcc; I tt=AT(x); I flg=AFLAG(x); if((tt^flg)&TRAVERSIBLE){AFLAG(x)=flg|(tt&RECURSIBLE); if(tt&RECURSIBLE&&!(flg&(AFNJA|AFSMM))&&AC(x)>=2&&AC(x)<0x3000000000000000)*(I*)0=0; jtra(jt,(x),tt);}; *Zcc=(c+1)&~ACINPLACE;}}
...
#define ra0(x)                      {I tt=AT(x); I flg=AFLAG(x); if((tt^flg)&RECURSIBLE){AFLAG(x)=flg|(tt&RECURSIBLE); if(!(flg&(AFNJA|AFSMM))&&AC(x)>=2&&AC(x)<0x3000000000000000)*(I*)0=0; jtra(jt,(x),tt);}}
#else
