...
#define behead(x)                   jtbehead(jt,(x),0)
#define beheadW(x)                  jtbehead((J)((I)jt+JTINPLACEW),(x),0)
...
#define canta(x,y)                  jtcanta(jt,(x),(y),0)
...
#define dfv1(r,x,y)                 (r=((r=(y))?(FAV(r)->valencefns[0])(jt,(x),r,r):r))  // y is self, always a verb
                                              //  r must not = x.  self is evaluated only once.  If we call a modifier, set that flag
...
#define dgrade1(x)                  jtdgrade1(jt,(x),0) 
...
#define drop(x,y)                   jtdrop(jt,(x),(y),0)   
...
#define efr(ar,r)                   ({I z=((r)>(ar)?(ar):(r))+(REPSGN(r)&(ar)); (z<0)?0:z;})  // effective rank: ar is rank of argument, r is assigned rank of verb (may be negative), z becomes rank of argument cell
...
#define faprobe(x) __atomic_fetch_sub(&AC(x),1,__ATOMIC_ACQ_REL)  // update the usecount.  Result is old usecount, with free required if <2
#define faactionrfo(jt,x,rfoaction) {I Zc=AC(x); I tt=AT(x); if(likely(((Zc-2)|tt)<0) || (likely(!ACISPERM(Zc)) && (rfoaction, faprobe(x)<2)))jtfamf(jt,x,tt);}  // call if sparse or ending; never touch a PERM
...
#define faowed(x,Zc,tt) {if(withprob(((Zc-2)|tt)<0,0.2) || (likely(!ACISPERM(Zc)) && withprob(faprobe(x)<2,0.1)))jtfamf(jt,x,tt);}  // call if sparse or ending; never touch a PERM
...
#define faafterrav(x)  {I Zc=__atomic_load_n(&AC(x),__ATOMIC_ACQUIRE); I tt=AT(x); if(likely(!ACISPERM(Zc))){if(unlikely(Zc<2)||unlikely((tt|(faprobe(x)-2))<0)){if(unlikely(AFLAG(x)&AFVIRTUAL)){fa(ABACK(x)); tt=0;} jtfamf(jt,x,tt);}} }
...
#define grade1(x)                   jtgrade1(jt,(x),0)
...
#define indexof(x,y)                jtindexof(jt,(x),(y),0)
...
#define lamin1(x)                   jtlamin1(jt,(x),0)  
...
#define repeat(x,y)                 jtrepeat(jt,(x),(y),0) 
...
#define reshape(x,y)                jtreshape(jt,(x),(y),0)
#define reshapeW(x,y)               jtreshape((J)((I)jt+JTINPLACEW),(x),(y),0)
...
#define reverse(x)                  jtreverse(jt,(x),0) 
...
#define tail(x)                     jttail(jt,(x),0)    
#define take(x,y)                   jttake(jt,(x),(y),0)   
