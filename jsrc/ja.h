...
#if MEMAUDIT&2
#define ra(x)                       {I c=AC(x); if(!ACISPERM(c)){I tt=AT(x); FLAGT flg=AFLAG(x); if((tt^flg)&TRAVERSIBLE){AFLAG(x)=flg|=(tt&RECURSIBLE); if(tt&RECURSIBLE&&!(flg&(AFNJA))&&AC(x)>=2&&AC(x)<0x3000000000000000)*(volatile I*)0=0; jtra(jt,(x),tt);}; AC(x)=(c+1)&~ACINPLACE;}}
...
#define ra0(x)                      {I tt=AT(x); FLAGT flg=AFLAG(x); if((tt^flg)&RECURSIBLE){if(flg&AFVIRTUAL){RZ((x)=realize(x)); flg=AFLAG(x);} AFLAG(x)=flg|=(tt&RECURSIBLE); if(!(flg&(AFNJA))&&AC(x)>=2&&AC(x)<0x3000000000000000)*(volatile I*)0=0; jtra(jt,(x),tt);}}
#else
...
#define thorn1main(x)               jtthorn1main(jt,(x))  
