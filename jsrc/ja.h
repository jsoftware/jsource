...
#define faactionrescindable(jt,x, nomfaction) {fadecr(x) I tt=AT(x); Zc=REPSGN(Zc-1); if(unlikely(((tt&=TRAVERSIBLE)&(Zc|~AFLAG(x)))!=0))if(unlikely(jtfa(jt,(x),tt)!=0))Zc=0; if(likely(Zc!=0)){jtmf(jt,x);} nomfaction}
...
// obsolete #define probe(x,y,z,w)              jtprobe(jt,(x),(y),(z),(w))
// obsolete #define probedel(x,y,z,w)           jtprobedel(jt,(x),(y),(z),(w))
...
// obsolete #define syrd1(x,y,z,w)              jtsyrd1(jt,(x),(y),(z),(w))
#define syrdforlocale(x)            jtsyrdforlocale(jt,(x))
// obsolete #define syrd1forlocale(x,y,z,w)     jtsyrd1forlocale(jt,(x),(y),(z),(w))
