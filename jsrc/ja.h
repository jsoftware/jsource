...
#else
#define ra(x)                       {if(x){I* Zcc=&AC(x); I c=*Zcc; I tt=AT(x); UI4 flg=AFLAG(x); if((tt^flg)&TRAVERSIBLE){AFLAG(x)=flg|(tt&RECURSIBLE); jtra(jt,(x),tt);}; *Zcc=(c+1)&~ACINPLACE;}}
// If this is a recursible type, make it recursible if it isn't already, by traversing the descendants.  This is like raising the usecount by 0.
#define ra0(x)                      {I tt=AT(x); UI4 flg=AFLAG(x); if((tt^flg)&RECURSIBLE){AFLAG(x)=flg|(tt&RECURSIBLE); jtra(jt,(x),tt);}}
#endif
#endif
