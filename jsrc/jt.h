...
typedef struct __attribute__((aligned(CACHELINESIZE))) {
 JOB *ht[2];  // queue head/tail.  When empty, ht[0] is 0 and ht[1] points to ht[1] as a harmless store location.  The job MUST be on a ABDY boundary, because LSBs are used as a lock.  Modified only when the job lock is held
...
// obsolete  C pppprunning;   // set while pppp is parsing the sentence.  Unsupported forms must fail then
// 6 bytes free
...
// end of cacheline 6
 ...
 A repato; // outgoing repatriation chain; chain of objects which all belong to the same thread.  AAV0(repato) points to the last link in the chain, and AN(repato) is the cumulative #bytes in the chain
