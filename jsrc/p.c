...
static I NOINLINE jtis(J jtfg,A n,A v,A symtab){F12IP;
 ...
 if(unlikely(AT(n)==BOX+BOXMULTIASSIGN)){   // test both bits, since BOXMULTIASSIGN has multiple uses
  ...
  if(AN(n)==1)n=AAV(n)[0];  // if there is only 1 name, treat this like simple assignment to first box, fall through
   ...
   if(likely(((AR(v)^1)+(~AT(v)&BOX))==0)){A *nv=AAV(n), *vv=AAV(v); DO(AN(n), jtsymbis(jtfg,nv[i],C(vv[i]),symtab);)}  // v is boxed list
...
// if(AC(w)<0&&!(AFLAG(w)&AFVIRTUAL)&&((*AZAPLOC(w)!=0&&*AZAPLOC(w)!=w)))SEGFAULT;  // if no zaploc for inplaceable block, error
 ...
 case BOXX:
  ...
  if(!(AFLAG(w)&AFNJA)&&!((AFLAG(w)&AFUNAUDITABLE)&&(AFLAG(w)&AFVIRTUAL))){A*wv=AAV(w);
   I acbias=(AFLAG(w)&BOX)!=0; A bv;  // subtract 1 if recursive; value of 
     // AT of ) looks like a pyx, so we have to test
   if(AFLAG(w)&AFPRISTINE){DO(AN(w), if((bv=CNULLNOERR(QCWORD(wv[i])))&&AT(bv)!=RPAR)if(!((AT(bv)&DIRECT)>0))SEGFAULT;)}  // wv[i]&&(AC(w)-acbias)>1|| can't because other uses may be not deleted yet
   {DO(AN(w), if((bv=CNULLNOERR(QCWORD(wv[i])))&&AT(bv)!=RPAR)auditblock(jt,bv,nonrecur,0););}
  }
  break;
...
A jtparsea(J jtfg, A *queue, I nwds){F12IP;PSTK *stack;
 ...
 if(likely(nwds>1)) {  // normal case where there is a fragment to parse - we get the values needed by the 1-word path loaded before the pipeline break
  ...
  //  19 free
  //  18  free
  //  19-26 AR flag from symtab, which cannot change during execution since it comes from the executing explicit defn.  Only ARLCLONED is used, others are garbage at startup
  //  16 always 1
...
#define STACK0PTISCAVN PTISCAVN(stack0pt)  // must be above bit 16
  ...
  if(unlikely((intptr_t)stack-(intptr_t)currstk<(nwds+BACKMARKS+FRONTMARKS+PSTACKRSV+1+1)*(intptr_t)sizeof(PSTK))){A sa;
   ...
   // We are taking advantage of the fact that NORMAH is 7, and thus a rank-1 array is aligned on a boundary of its size
  ...
  I4 flgsfor3=nwds+FLG1+(0b11LL<<CONJX);
  I4 flgsfor4=flgsfor3+(0b11LL<<CONJX);   // flags to pull 3/4 words
...
firstword: ;  // come here first time, while pt0ecam settles
     ...
     if(((I)y&QCISLKPNAME)){
      ...
      // Resolve the name.  If the name is mnuvxy, always resolve the name to its current value;
      ...
      if(!(pt0ecam&(ARLCLONED<<LOCSYMFLGX))){  // primary table?  This branch prediction will be unchanged as long as we are in the same definition
       ...
       if(likely(QCPTYPE(lka)!=0)){y=lka; goto finlocal;}   // normal case of lookaside found.  Use it
      ...
      if(likely(buck>0)){  // buckets but no symbol - must be global, or cloned symtab - but not synthetic new name.
       ...
       else y=probelocalbuckets(sympv,y,LXAV0(jt->locsyms)[buck],bx);  // must be unallocated name and there has been a surprise write to the cloned symtab.  Rare indeed.  If not found, it's global - restore y
       if(unlikely(y==0)){y=*(volatile A*)queue;goto rdglob;}  // if the local search came up undefined, switch over to global (must reinit y)
...
rdglob: ;   // here we could not use buckets or lookaside.  Resort to full search (without buckets)
       ...
       if(NAV(y)->cachedref!=0){  // if the user doesn't care enough to turn on caching, performance must not be that important
        ...
        goto advcheck; // take its type, proceed.  We skip the FAOWED issues (FAOWED must be clear in the cached addr)
       }
       y=syrdnobuckets(y,jt->locsyms);    // do full symbol lookup, knowing that we have checked for buckets already.  Error if not defined.  Result has QCFAOWED semantics
       if(unlikely(y==0))goto undefname;
      }

      // end of looking at local/global symbol tables.  Examine the value and the name for special processing
      ...
      if(likely(1)){
       ...
       if((pt0ecam&(NAMEBYVALUE>>(NAMEBYVALUEX-NAMEFLAGSX)))+((I)y&QCNOUN)){   // use value if noun or special name, or name_:
        ...
        if(likely((I)y&QCNOUN))goto endname;  // we are almost always on a noun - otherwise u/v, or name_: - so worth skipping the adverb check
...
advcheck:;
      ...
      pt0ecam&=~((1ULL<<(CONJX+2-QCADV))<<QCPTYPE(y));  // If ADV, turn off bit 31 (request for 4).  This could turn off 28 or 29 but not 27 or 30
endname: ;
     }else{
     ...
     // names have been resolved.  Put the value onto the execution stack
     ...
     I tx=QCPTYPE(y); I savy=(I)y;  // extract the 4-bit parsing-type index, and save y to allow early load
...
#define NEXTY queue[0]
  ...
  // Now execute fragments as long as there is one to execute.
   for(;1;pt0ecam&=~FLGPMSK) {   // pmask must be cleared on each execution pass
...
anchoredip:;  // here when we have detected that an anchored name is inplaceable
      ...
      A *tpopw=AZAPLOC(QCWORD(arg2)); tpopw=(A*)((I)tpopw&REPSGN(AC(QCWORD(arg2))&((AFLAG(QCWORD(arg2))&(AFVIRTUAL|AFUNINCORPABLE))-1))); tpopw=tpopw?tpopw:(A*)STKNAMED; tpopw=ISSTKNAMED(arg2)?(A*)arg2:tpopw; tpopw=pt0ecam&FLGPLINE2?(A*)tpopw:(A*)STKNAMED;
...
RECURSIVERESULTSCHECK
      ...
      ramkrecursv(y);  // force recursive y - reloads y
...
RECURSIVERESULTSCHECK
       ...
       pt0ecam|=PTISCAVN(stack[3].pt)?0:(FLGPMONAD|FLGPINCR); // set initial value of loop counter/flag, cleared by stacker
...
gotlocalval1:;  // local fetches can skip the tpush.  Value in y
