...
static PSTK* jtis(J jt){A f,n,v;B ger=0;C c,*s;PSTK* stack=jt->parserstkend1; 
 ...
 if(jt->assignsym){symbis(n,v,(A)AT(stack[1].a));}   // Assign to the known name.  Pass in the type of the ASGN
  ...
  else {ASSERT(1==AR(n),EVRANK); jt->pre=ger?jtfxx:jtope; rank2ex(n,v,0L,-1L,-1L,-1L,-1L,jtisf);}
 }
...
F1(jtparsea){PSTK *stack;A z,*v;I es,i,m,maxnvrlen; L* s;  // symbol-table entry
 ...
 #define ST(i) AT(stack[i].a)
   ...
   if(i&3){A y;  // if not PNOMATCH... (we test this way to start calculating the token-in-error offset)
