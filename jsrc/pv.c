...
static F1(jtvtokens){F12IP;A t,*y,z;I n,*s;TA*x;
...
static F1(jtcfn){F12IP;I j; R !AR(w)&&INT&AT(w)&&(j=AV(w)[0],-9<=j&&j<=9)?FCONS(w):qq(w,ainf);}
...
TACT(jtvhook){TA z={0,0}; if(CHK2)z.a=hook(stack[b].a,stack[e].a,0); R z;}
...
#define ASGRCOMMON(w,x,s,test) ({A val=(s); test if(AFLAG(w)&RECURSIBLE){if(val)ra(val); if(FAV(w)->fgh[x]!=0){A t=FAV(w)->fgh[x]; FAV(w)->fgh[x]=0; fana(t);} } FAV(w)->fgh[x]=val; FAV(w)->fgh[x];})  // t for audits
...
F1(jtvtrans){F12IP;PROLOG(0053);A locsyms,y,z=0;I c,i,ttabi;TA ttab[NTTAB];
 ...
 for(i=0;!z&&2>i;++i){
  ...
  WITHJTJERROFF(z=jttparse(jt,y,locsyms,tmonad,0==i,ttab,&ttabi,c);)
