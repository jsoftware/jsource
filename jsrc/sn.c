...
F1(jtex){F12IP;A*wv,y,z;B*zv;I i,n;
 ...
 for(i=0;i<n;++i){
  ...
  if(y&&(locfound=syrdforlocale(y))){
   ...
   jtprobedel((J)((I)jt+NAV(y)->m),NAV(y)->s,NAV(y)->hash,locfound);  // delete the symbol (incl name and value) in the locale in which it is defined.  if we delete an ACV, probede invalidates cached references
