...
A jtfindnameinscript(J jt,C *script, C *name, I pos){
 ...
 for(step=0;step<2;++step){
  ...
  case 0: ; A w=nfs(strlen(name),name); L *sym; if((sym=syrd(w,jt->locsyms))!=0){target=namerefacv(w,sym->val); fa(sym->val) if(target&&LOWESTBIT(AT(target))&pos)R target;}  // there is always a ref, but it may be to [:.  Undo ra() in syrd
  }
