...
// obsolete   case 0: ; A w=nfs(strlen(name),name); A val; if((val=QCWORD(syrd(w,jt->locsyms)))!=0){target=QCWORD(namerefacv(w,val)); fa(val) if(target&&LOWESTBIT(AT(target))&pos)R target;}  // there is always a ref, but it may be to [:.  Undo ra() in syrd
