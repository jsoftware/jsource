...
I jtmaxtype(J jt,I s,I t){
 ...
 if(unlikely(ISSPARSE(s|t))){ASSERT(!((s|t)&(C2T|C4T|XNUM|RAT)),EVDOMAIN); R SPARSE|BIT(resultbit);}  // If either operand sparse, return sparse version
...
F1(jtvib){F12IP;A z;I i,n,*zv;
 ...
 I p=-IMAX,q=IMAX;
 RANK2T oqr=jt->ranks; RESETRANK;  // scafrk needed?
...
A jtfindnameinscript(J jt,C *script, C *name, I pos){
 ...
 for(step=0;step<2;++step){
  ...
  case 0: ; A w=nfs(strlen(name),name,0); A val; if((val=syrd(w,jt->locsyms))!=0){target=QCWORD(namerefacv(w,val)); if(target&&LOWESTBIT(AT(target))&pos)R target;}  // there is always a ref, but it may be to [:.  Undo ra() in syrd
  }
