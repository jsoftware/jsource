...
A jtcharmap(J jt,A w,A x,A y){A z;B bb[256];I k,n,wn;UC c,*u,*v,zz[256];
 ...
 k=256; memset(bb,C0,256); if(n<AN(y))memset(zz,UAV(y)[n],256);  // bb is array telling which input chars are in x; zz is result char to map for given input byte.  If not exact mapping, init z to the 'not found' char
