...
static B eqv(I af,I wf,I m,I n,I k,C* RESTRICT av,C* RESTRICT wv,B* RESTRICT z,B b1){
 ...
 if(k==((k&-k)&(2*SZI-1))){
  ...
  compfn[CTTZI(k)][b1](n==1?~m:2*(n^REPSGN(n))+SGNTO0(n),z,wv,av,m,0);  // no jt for these routines.  Swap wv/av because n is positive, which will mean repeat the first arg
...
static B eqvfl(I af,I wf,I m,I n,I k,D* RESTRICT av,D* RESTRICT wv,B* RESTRICT z,B b1,J jt){
 ...
 if(k==1){
  ...
  (b1?eqDD:neDD)(n==1?~m:2*(n^REPSGN(n))+SGNTO0(n),z,wv,av,m,jt);  // Swap wv/av because n is positive, which will mean repeat the first arg
  R 0;  // return value immaterial
 }
 __m256d u,v;
 // prep for each compare loop
