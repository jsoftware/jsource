...
#include <fenv.h>
takestats(static int stats[0x40]={0};)
...
bitwiseexit:;
 ...
 I rc=cirDD(~1,cirvals+2,cirvals,cirvals+1,0,jt); rc=rc<0?EWOVIP+EWOVIPMULII:rc;  // run the routine
...
/* non-atomic functions      */ [0]={
 {{err00,0}, {err00,0}, {err00,0},                                /* BB BI BD              */
  {err00,0}, {err00,0}, {err00,0},                                /* IB II ID              */
  {err00,0}, {err00,0}, {err00,0},                                /* DB DI DD              */
  {err00,0}, {err00,0}, {err00,0}, {err00,0}, {err00,0}, {err00,0}},                        /* XX QQ I2 I4 E Z    (prority order)    */
  &rpsnull},
...
takestats(++stats[0x11];)
  ...
  if((I)jtfg&JTSPARSEARG){   // Handle sparse args
   // Sparse.  Agreement first, copied from below
   ...
   I awcr=awr-afwf;  // cell ranks
   R vasp(a,w,va2ctoc[vandx/sizeof(VA)],adocvfn==(VF)err00?0:adocvfn,cv,isatype(cv)?atype(cv):0,rtype(cv),afwf>>RANKTX,awcr>>RANKTX,(RANKT)afwf,(RANKT)awcr,MAX(afwf>>RANKTX,(RANKT)afwf),MAX(awcr>>RANKTX,(RANKT)awcr));  // handle sparse arrays separately.
  }
...
// obsolete &&likely(!((I)jtfg&JTSPARSEARG)))
  ...
  if(TYPESNE(AT(a),t)){A cz=cvt(t|(cv&XCVTXNUMORIDEMSK),a); if(likely(cz!=0)){a=cz; jtfg=(J)((I)jtfg|JTINPLACEA);}else{if(jt->jerr!=EVDOMAIN)R 0; RESETERR adocvfn=err00; jtfg=(J)((I)jtfg&~JTINPLACEA);}}
  if(TYPESNE(AT(w),t)){A cz=cvt(t|(cv&XCVTXNUMORIDEMSK),w); if(likely(cz!=0)){w=cz; jtfg=(J)((I)jtfg|JTINPLACEW);}else{if(jt->jerr!=EVDOMAIN)R 0; RESETERR adocvfn=err00; jtfg=(J)((I)jtfg&~JTINPLACEW);}}
 }

 // Now that we know a/w for sure, calculate inplaceability.  This requires settlement of memory fetches, so we do it here long before we test it, so as to reduce the cost of a misbranch (which is considerable: 25% of
 // operations in the tests are inplaceable).
takestats(if(cv&JTINPLACEW)++stats[0x30]; if(cv&JTINPLACEA)++stats[0x31]; if(cv&JTINPLACEW&&cv&JTINPLACEA)++stats[0x32]; )
takestats(if(AC(w)<0)++stats[0x33]; if(AC(a)<0)++stats[0x34]; if(AC(w)<0&&AC(a)<0)++stats[0x35]; )
takestats(if(AC(w)<0&&cv&JTINPLACEW)++stats[0x36]; if(AC(a)<0&&cv&JTINPLACEA)++stats[0x37]; if(AC(w)<0&&cv&JTINPLACEW&&AC(a)<0&&cv&JTINPLACEA)++stats[0x38];)
takestats(if(jt->zombieval!=0)++stats[0x39];)
takestats(if(jt->zombieval==a)++stats[0x3a]; if(jt->zombieval==w)++stats[0x3b]; if(jt->zombieval==a&&jt->zombieval==w)++stats[0x3c];) 
 I1 ipfromaw=2*(I1)(SGNTO0(AC(a))|(a==jt->zombieval))+(I1)(SGNTO0(AC(w))|(w==jt->zombieval));  // inplaceable, bit 1=a, bit 0=w.  98% of the inplacing comes from AC
 cv&=(I)((I1)(intptr_t)jtfg&ipfromaw)|~(JTINPLACEA+JTINPLACEW);  // If function doesn't support inplacing, remove it from the argument.  cv has not settled yet
...
#else
   cv&=~(((uint32_t)atommsk*(uint32_t)(BIT(32-2-22)+BIT(32-3-14)))>>(32-3));   // 32-bit computation to discard upper bits.  Move bit 22 to bit 30, bit 14 to bit 29, shift down to 0-1
#endif
...
takestats(++stats[0x3d];)
  if(withprob(!(cv&VIPOLOOPREQD),0.8))goto noallonoloop; else goto noalloloop;
 }else{
allocate:;  // come here if no inplaceable block could have the type changed
takestats(++stats[0x3e];)
  ...
  fru.fr=fr;  // save fr where we can get to the lanes fr free
#if SY_64
  ASSERT((UI)LANE(fr,ZRANK)<=(UI)RMAX,EVLIMIT) ASSERT((UI)zn<=2147483647,EVLIMIT)   // verify size & rank are in limits
#else
  ASSERT(((I)bytes>(I)(zn)&&(I)(zn)>=(I)0)&&!(LANE(fr,ZRANK)&~RMAX),EVLIMIT)
#endif
...
takestats(if(adocvfn==statsoldadocvfn[0])++stats[0x2f]; statsoldadocvfn[0]=adocvfn;)
   ...
   // outer loop needed (except in rare case when it was elided by loop migration)
...
takestats(if(adocvfn==statsoldadocvfn[1])++stats[0x2f]; statsoldadocvfn[1]=adocvfn;)
   ...
   //jj counts the inners and aawwzknfxrz[9] has the starting offset of the last inner loop of all.  aawwzknfxrz[0,1] are the cell-size of a for the outer loop, aawwzknfxrz[2,3] are for w;
   ...
   do{
    ...
    lrc=((AHDR2FN*)adocvfn)(m,zvu,avu,wvu,n,jt);    // run one section.  Result is EOK normally, otherwise error code, as examined below.  After the first time it doesn't matter whether adocvfn is in a reg, since the branch is predicted
...
retrylrc:;  // exit to here with lrc set to error code when there is an error
  ...
  // ********* error recovery starts here **********
   ...
   n=(m>>1)^-(m&1); n=m<0?1:n;  // now n is the number of loops, but complemented if x repeats.  If m is neg (no repeat), it is 1; otherwise m>>1, complemented if x is repeated
   I temp=aawwzknfxrz[5]; m=(m=~m)<0?temp:m;  // m is the length of a single loop (if m neg), or the number of loops if nested
  ...
  // Set up pointers etc for the overflow handling.  Set b=1 if w is taken for the x argument to repair
   if(lrc==EWOVIP+EWOVIPMULII){D *zzvd=(D*)zzv; I *zvi=IAV(z);
    ...
    {C *av, *zv=CAV(z);
     ...
     NOUNROLL while(1){(repairfn)(m,zzv,av,zv,n,jt); if((I)zv==mend)break; aawwzknfxrz[7]=--jj; jj=REPSGN(jj); zv+=aawwzknfxrz[4]; zzv+=zzk; av+=aawwzknfxrz[2*nipw+1+jj]; jj=aawwzknfxrz[7+jj];}  // jj1 is -1 on the last inner iter, where we use outer incr
    }
   }
...
DF2(jtfslashatg){F12IP;A fs,gs,y,z;B b;C*av,*wv;I ak,an,ar,*as,at,m,
 ...
 adocv=var(FAV(gs)->lu2.lc&0x7f,at,wt); ASSERT(adocv.f,EVDOMAIN); yt=rtype(adocv.cv);  // get type info on g
 adocvf=var(FAV(y)->lu2.lc&0x7f,yt,yt); ASSERT(adocvf.f,EVDOMAIN); zt=rtype(adocvf.cv);   // get type info on f/
 ...
 rc=((AHDR2FN*)adocv.f)(n==1?~m:2*(n^REPSGN(n))+SGNTO0(n),zv,av,wv,m,jt); rc=rc<0?EWOVIP+EWOVIPMULII:rc;  // create first result-cell of g
 DQ(nn-1, av-=ak; wv-=wk; I lrc; lrc=((AHDR2FN*)adocv.f)(n==1?~m:2*(n^REPSGN(n))+SGNTO0(n),yv,av,wv,m,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc;
    lrc=((AHDR2FN*)adocvf.f)(~zn,zu,yv,zv,0,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc; {C* ztemp=zu; zu=zv; zv=ztemp;});  // p==1 means result goes to ping buffer zv
...
// opchar is the VA2C* code for the operation
VA2 jtvar(J jt,I opchar,I at,I wt){   // scaf parm should be lu2.lc not self?
 ...
 if(likely(!jerr)){
  ...
  if(withprob((t&(NOUN&~NUMERIC)),0.8)){I argtypes;   // comparison of chars or boxes, most likely, unless the user is into sparse or exotic types
   ...
   }else{retva2.f=err00; retva2.cv=0;}  // if not equality comparison, it's a domain error
   R retva2;
  }
  ...
  retva2.cv=0;  // error if not filled in
  switch(opchar){
...
// the following errors are normally retryable in place.  We keep the alternate code for sparse
  ...
  if(likely(retva2.cv)){RESETERR}else{retva2.f=err00; if(jerr>NEVM){RESETERR jsignal(EVSYSTEM);}}  // system error if unhandled exception.  Otherwise reset error only if we handled it
  R retva2;
 }
}
