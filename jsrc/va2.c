...
static A jtva2(J jt,AD * RESTRICT a,AD * RESTRICT w,AD * RESTRICT self,UI allranks){  // allranks is argranks/ranks
 ...
 I aawwzknffx[9];  // a outer/only, a inner, w outer/only, w inner, z, n parm to ado, mf, nf, nf wkarea
 F2PREFIP;
 {I at=AT(a);
  I wt=AT(w);
 ...
 {//I *as = AS(a); I *ws = AS(w);
  if(likely((allranks&RANK2TMSK)==0)){ // rank 0 0 means no outer frames, sets up faster
   ...
   if(likely(jtinplace!=0)){  // nonsparse
    ...
    zn=raminusw<0?zn:an; aawwzknffx[5]=m=raminusw<0?an:m;  // zn=# atoms in higher-rank operand, m=#atoms in lower-rank
   ...
   // notionally we now repurpose fr to be frame/rank, with the frame 0
    ...
    // parm aawwzknffx[5] is orig m, i. e. the length of the inner or only loop.
    ...
    m=~m;  // parm m if there is only 1 loop - the length of the loop, complemented as a flag.  The aawwzknffx[5] value is unused in this case
    ...
    aawwzknffx[0]=fr; aawwzknffx[1]=shortr; fr=shortr>fr?shortr:fr; mf=0; nf=0;
   }
   ...
   if(likely(jtinplace!=0)){  // If not sparse... This block isn't needed for sparse arguments, and may fail on them.
    ...
    aawwzknffx[4]=zn<<bplg(cvt!=0?cvt:(I)jtinplace&VCOPYA?at:wt);  // calc result-cell size and move it out of registers
    ak<<=bplg(at); wk<<=bplg(wt);  // convert cell sizes to bytes
    aawwzknffx[0]=ak; aawwzknffx[2]=wk; ak=((I)jtinplace&VIPWFLONG)?0:ak; wk=((I)jtinplace&VIPWFLONG)?wk:0; aawwzknffx[1]=ak; aawwzknffx[3]=wk;  // set inner cell size for last followed by non-last.  Last is 0 for a repeated cell ak/wk free
...
// this shows the fix   if(ICMP(as+af,ws+wf,MIN(acr,wcr))){if(1||zn)ASSERT(0,EVLENGTH)else r = 0;}
    ...
    }else{aawwzknffx[7]=nf-1;}     // All 4 loops (normal case since rank given).  nf is outer loop repeat count-1
    // Use new semantics for m and n
    aawwzknffx[5]=m;  // parm n is orig m, i. e. the length of the inner or only loop.
    ...
    fr=acr<wcr?wcr:acr; I f=(af<wf)?wf:af; fr+=(f<<RANKTX)+f; aawwzknffx[0]=acr; aawwzknffx[1]=wcr; mf=af; nf=wf;
    // For sparse, repurpose aawwzknffx/mf/nf to hold acr/wcr/af/wf, which we will pass into vasp.  This allows acr/wcr/af/wf to be block-local
 ...
 if(likely(jtinplace!=0)){   // if not sparse...
  ...
  if(unlikely(((I)jtinplace&VARGMSK)!=0))if(likely(zn>0)){  // input conversion required (rare), and the result is not empty
   ...
   if(TYPESNE(t,at)){I ba=bplg(at); aawwzknffx[0]=(aawwzknffx[0]>>ba)<<bt; aawwzknffx[1]=(aawwzknffx[1]>>ba)<<bt; RZ(a=cvt(t|((I)jtinplace&XCVTXNUMORIDEMSK),a)); jtinplace = (J)(intptr_t)((I)jtinplace | (((I)jtinplace>>2)&JTINPLACEA));}
   if(TYPESNE(t,wt)){I bw=bplg(wt); aawwzknffx[2]=(aawwzknffx[2]>>bw)<<bt; aawwzknffx[3]=(aawwzknffx[3]>>bw)<<bt; RZ(w=cvt(t|((I)jtinplace&XCVTXNUMORIDEMSK),w)); jtinplace = (J)(intptr_t)((I)jtinplace | (((I)jtinplace>>2)&JTINPLACEW));}
...
//                                     frame loc     shape of long frame             len of long frame  cellshape       longer cellen 
  ...
  // force the compiler to spill aawwzknffx by using address arithmetic.
  {I rc=((I)jtinplace&VRI+VRD)+EVOK; I mulofloloc;  // init good rc, and transfer output conversion to it, freeing up inplace (we know that anything needing conversion will not need retries); and number of good results before we hit integer overflow on multiply
   {  // lowest rc from the executed sections
    ...
    // Call the action routines: nf,mf, etc must be preserved in case of repair
     // mf has the total number of calls.  nf is 1 less than the number of calls with a repeated cell.  aawwzknffx[0,1] are the cell-size of a for the outer loop, aawwzknffx[2,3] are for w;
     // but aawwzknffx[1,3] have 0 in a repeated argument.  aawwzknffx[1,3] are added for each inner iteration, aawwzk[0,2] at the end of an inner cycle
     ...
     // aawwzknffx[5] is the number of outer loops, used only if m>0.  n*m cannot=0. 
     // Each release, monitor that clang brings adocvfn into register early to advance the expected misprediction.
    I i=mf; I jj=aawwzknffx[7];  // number of outer-outer loops, number-1 of each outer-inner loop
    lp000: {I lrc=((AHDR2FN*)adocvfn)AH2A(aawwzknffx[5],m,av,wv,zv,jt);    // run one section.  Result is EOK normally, otherwise error code, as examined below
     if(unlikely(lrc!=EVOK)){
      // section did not complete normally.
      if(unlikely(lrc<0)){I absn=(m>>1); absn=m<0?1:absn; mulofloloc=(mf-i)*aawwzknffx[5]*absn+~lrc; rc=EWOVIP+EWOVIPMULII; goto lp000e;}  // integer multiply overflow.  ~lrc is index of failing location; create global failure index.  Abort the computation to retry
     ...
     if(unlikely(--i!=0)){aawwzknffx[8]=--jj; jj=REPSGN(jj); zv+=aawwzknffx[4]; av+=aawwzknffx[1+jj]; wv+=aawwzknffx[3+jj]; jj=aawwzknffx[8+jj];  goto lp000; }  // jj1 is -1 on the last inner iter, where we use outer incr
    }
    lp000e: ;
   }
...
// not yet    if(rc==EVOKCLEANUP){jt->mulofloloc=0; RETF(z);}  // if multiply that did not overflow, clear the oflo position for next time, and return
    ...
    if(sizeof(D)==sizeof(I)){zz=z; MODBLOCKTYPE(zz,FL); zzk=aawwzknffx[4];   // shape etc are already OK
    }else{GATV(zz,FL,AN(z),AR(z),AS(z)); zzk=aawwzknffx[4]*(sizeof(D)/sizeof(I));}
    ...
    if(rc==EWOVIP+EWOVIPMULII){D *zzvd=(D*)zzv; I *zvi=IAV(z);
     ...
     {C *av=CAV(a); C *wv=CAV(w);
      I i=mf; I jj=aawwzknffx[7]; 
      while(1){
       tymesIIO(n,m,(I*)av,(I*)wv,(D*)zzv,mulofloloc); if(!--i)break;
       mulofloloc-=m*(n^REPSGN(n)); zzv+=zzk; aawwzknffx[8]=--jj; jj=REPSGN(jj); av+=aawwzknffx[1+jj]; wv+=aawwzknffx[3+jj]; jj=aawwzknffx[8+jj];  // jj1 is -1 on the last inner iter, where we use outer incr
      }
     }
     ...
     {C *av, *zv=CAV(z);
      ...
      // av may have repeats.  Repeats before the function call are handled exactly as the first time through, by using aawwzknffx.
      ...
      I i=mf; I jj=aawwzknffx[7]; NOUNROLL while(1){(repairfn)AH2A(n,m,av,zv,zzv,jt); if(!--i)break; zv+=aawwzknffx[4]; zzv+=zzk; aawwzknffx[8]=--jj; jj=REPSGN(jj); av+=aawwzknffx[2*nipw+1+jj]; jj=aawwzknffx[8+jj];}  // jj1 is -1 on the last inner iter, where we use outer incr
     }
    }
 ...
 // sparse processing:
 }else{z=vasp(a,w,FAV(self)->id,aadocv->f,aadocv->cv,atype(aadocv->cv),rtype(aadocv->cv),mf,aawwzknffx[0],nf,aawwzknffx[1],fr>>RANKTX,(RANKT)fr-(fr>>RANKTX)); if(!jt->jerr)R z;}  // handle sparse arrays separately.
