...
static A jtvasp0(J jt,A a,A w,VF ado,I cv,I t,I zt){A e,x,xx,y,z,ze,zx;B b;I n;P*p,*zp;
 ...
 GA00(ze,zt,1,0); I rc;     ASSERT(((EVOK|EVNOCONV)&(rc=((AHDR2FN*)ado)(~1,AV0(ze),b?AV(e):AV(y),b?AV(y):AV(e),0,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc)),rc);
 GA(zx,zt,n,AR(x),AS(x)); if(n)ASSERT(((EVOK|EVNOCONV)&(rc=((AHDR2FN*)ado)(2*n+1-b,AV(zx),b?AV(x):AV(y),b?AV(y):AV(x),1,jt), rc=rc<0?EWOVIP+EWOVIPMULII:rc)),rc);  // was !b
...
#define FLUSH  if(d){c=d*yc; ICPY(zyv,u-c,c); I lrc=((AHDR2FN*)ado)(~(d*xc),zxv,axv-d*ak,wxv-d*wk,0,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc; \
                     zxv+=d*zk; zyv+=c; d=0;}
...
static A jtvaspeq(J jt,A a,A w,C id,VF ado,I cv,I t,I zt,I f,I r){A ae,ax,ay,we,wx,wy,z,za,ze,zx,zy;
 ...
 while(m>i&&n>j){
  c=0; DO(yc, if(c=u[i]-v[i])break;);
  if(0>c)   {FLUSH; if(wb){ICPY(zyv,u,yc); I lrc=((AHDR2FN*)ado)(2*xc,zxv,axv,wev,1,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc; zxv+=zk; zyv+=yc;} ADVA;}
  else if(c){FLUSH; if(ab){ICPY(zyv,v,yc); I lrc=((AHDR2FN*)ado)(2*xc+1,zxv,aev,wxv,1,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc; zxv+=zk; zyv+=yc;} ADVW;}
  else      {++d; ADVA; ADVW;}
 }
 FLUSH;
 if     (wb&&m>i){c=m-i; ICPY(zyv,u,c*yc); I lrc=((AHDR2FN*)ado)(2*c*xc,zxv,axv,wev,1,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc;}
 else if(ab&&n>j){c=n-j; ICPY(zyv,v,c*yc); I lrc=((AHDR2FN*)ado)(2*c*xc+1,zxv,aev,wxv,1,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc;}
 GA00(ze,zt,1,0); I lrc=((AHDR2FN*)ado)(~1,AV0(ze),aev,wev,0,jt); lrc=lrc<0?EWOVIP+EWOVIPMULII:lrc; rc=lrc<rc?lrc:rc;
...
A jtvasp(J jt,A a,A w,C id,VF ado,I cv,I t,I zt,I af,I acr,I wf,I wcr,I f,I r){A fs,z;
 ...
 if(af!=wf){RZ(fs=ds(id)); R sprank2(a,w,fs,acr,wcr,FAV(fs)->valencefns[1]);}
