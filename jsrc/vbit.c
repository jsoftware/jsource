...
static F1(jtbitslplus){A t,z;I i,m,mm,n,n1,p,q,*u,wr,*ws,*zv;UC c,*vc,*wv;UI*v,*v0;static UI ptab[256];
 ...
 GATV(z,INT,n,wr-1,1+ws); zv=AV(z); mvc(SZI*n,zv,1,MEMSET00);
 if(!n)R z;
 if(!m){mvc(n*SZI,zv,1,MEMSET00); R z;}
#if SY_64
 mm=255;
...
static F1(jtbitravel){A z;I c,m,rw,wn,wr,*ws;UC*wv,*zv;
 ...
 if(rw){I j,q,r,r1,t,ti;UC k,y;
  ...
  if(r&&t)*zv++=k;
  if(wn%BW)mvc((BW-wn%BW,zv,1,MEMSET00)/BB);
 }else MC(zv,wv,wn/BB);
 R z;
}    /* , bit array */
...
static F2(jtbitrepeat){A z;I c,c1,m,n,p,wr,wk,wt;UC*av,k;
 ...
 p=AR(w)?AS(w)[0]:1; m=AN(w)/p; c=wr?AS(w)[wr-1]:1; c1=(c+BW-1)/BW;
 wk=wt&BIT?SZI*c1*(m/c):m<<bplg(wt);
 ASSERT(n==p,EVLENGTH);
 p=bitsum(n,av);
 GA(z,wt,p*m,AR(w),AS(w)); AS(z)[0]=p;
