...
static A jtrinvip(J jt,A w,I n,I ncomp){PROLOG(0066);A ai,bx,di,z;I m;
 ...
 if(ISSPARSE(AT(w))){z=over(stitch(ai,bx),take(v2(n-m,-n),di));  // should copy this over w, inplace
  ...
  rightr=voidAV(di);
  DQ(n-m, mvc(leftlen,zr,1,MEMSET00); zr=(C*)zr+leftlen; MC(zr,rightr,rightlen); zr=(C*)zr+rightlen; rightr=(C*)rightr+rightlen;)
...
ARGCHK1(w);
 ...
 rightr=voidAV(l1);
 DQ(rw-m, mvc(leftlen,zr,1,MEMSET00); zr=(C*)zr+leftlen; MC(zr,rightr,rightlen); zr=(C*)zr+rightlen; rightr=(C*)rightr+rightlen;)
