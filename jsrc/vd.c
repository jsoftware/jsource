...
static A jtrinvip(J jt,A w,I n,I ncomp){PROLOG(0066);A ai,bx,di,z;I m;
 ...
 m=n>>1; I tom=PEXTN(0x01222100,(n&7)<<2,3); m=(m+tom<n)?m+tom:m;  // Minimize number of wasted multiply slots, processing in batches of 4
...
static F1(jtltqip){F12IP;PROLOG(0067);A l0,l1,y,z;
 ...
 I m=rw>>1; I tom=PEXTN(0x01222100,(rw&7)<<2,3); m=(m+tom<rw)?m+tom:m;  // Minimize number of wasted multiply slots, processing in batches of 4
 // construe w as w0 w1
