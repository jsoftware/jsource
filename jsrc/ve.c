...
INLINE I remii(I a,I b){if((a&(a-1))==0&&likely(a>=0))R b&(a-1); if(unlikely(a==-1))R 0; I r=b%a; I adj=a&REPSGN(a^b); R r+(r==0?0:adj);}  // a=nonneg power of 2 is fast (including a=0, which returns b).  b=-1 must give 0 with overflow on _1|IMIN
...
#define EVEN(mode,div) \
   ...
   __mmask8 mask=_bzhi_u64(0xff,tlen); \
...
F1(jtabase1){F12IP;A d,z;B*zv;I c,n,p,r,t,*v;UI x;
 ...
 if(!(t&INT)){
  ...
  d=IRS1(jthead,jt,z,1L,0L); if(0==i0(aslash(CMAX,ravel(d))))R d=IRS1(jtbehead,jt,z,1L,0L);
  RETF(z);
 }
