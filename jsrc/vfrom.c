...
F2(jtcreatedic){F12IP;A box,box1;  // temp for box contents
 ...
 A z; GAT0(z,BOX,20,1) AK(z)=offsetof(struct Dic,hash); AN(z)=8;  // allocate nonrecursive box, long enough to make the total allo big enough in 32- and 64-bit.  Then restrict to the boxes in case of error
 if(AT(a)&VERB){  // initial creation
  ...
  INCORPNV(box1); ((DIC*)z)->bloc.kshape=box1; ((DIC*)z)->bloc.ktype=t; I l=n<<bplg(t); ((DIC*)z)->bloc.kbytelen=n; // save shape & type; save #bytes in key
 ...
 *(I*)ev=maxeles-1; ((DIC*)z)->bloc.empty=box; // install end of chain loopback and point the dic to beginning of chain
 GA0(box,((DIC*)z)->bloc.ktype,maxeles*((DIC*)z)->bloc.kaii,1) INCORPNV(box) AS(box)[0]=maxeles; MCISH(AS(box)+1,IAV1(((DIC*)z)->bloc.kshape),AN(((DIC*)z)->bloc.kshape)) ((DIC*)z)->bloc.keys=box;   // allocate array of keys
 GA0(box,((DIC*)z)->bloc.vtype,maxeles*((DIC*)z)->bloc.vaii,1) INCORPNV(box) AS(box)[0]=maxeles; MCISH(AS(box)+1,IAV1(((DIC*)z)->bloc.vshape),AN(((DIC*)z)->bloc.vshape)) ((DIC*)z)->bloc.vals=box;   // allocate array of keys
...
B jtkeyprep(DIC *dic, void *k, UI8 n, I8 *s,J jt){UI8 i;
 UI8 hsz=dic->bloc.hashsiz; I kib=dic->bloc.klens; UI4 (*hf)()=dic->bloc.hashfn; C *hashtbl=CAV1(dic->bloc.hash);  // elesiz/hashsiz kbytelen/kitemlen
...
B jtgetslots(DIC *dic,void *k,I n,I8 *s,void *zv,J jt){UI8 i;
 ...
 UI8 hsz=dic->bloc.hashsiz; I kib=dic->bloc.klens; I (*cf)()=dic->bloc.compfn; C *hashtbl=CAV1(dic->bloc.hash);  // elesiz/hashsiz kbytelen/kitemlen
 k=(void*)((I)k+n*(kib>>32));  // move to end+1 key to save a reg by counting down
 C *kbase=CAV(dic->bloc.keys)-4*(kib>>32);  // address corresponding to hash value of 0.  Hashvalues 0-3 are empty/tombstone/birthstone and do not take space in the key array
 ...
 I vn=dic->bloc.vbytelen; C *vbase=CAV(dic->bloc.vals)-4*vn;  // size of a value; address corresponding to hash value of 0.  Hashvalues 0-3 are empty/tombstone/birthstone and do not take space in the value array
 DO(n, ASSERT(s[i]>1,EVINDEX) MC(zv,vbase+s[i]*vn,vn); zv=(void *)((I)zv+vn);)   // scaf JMC?
 R 1;
}

// get.  conjunction.  u is dic, v is hash/comp function.    w is keys
// This version for internal functions only
DF1(jtdicget){F12IP;A z;
 ARGCHK1(w)
...
static INLINE B jtputslots(DIC *dic,void *k,I n,void *v,I vn,I8 *s,J jt,I lv){I i;
 DICLKRWWT(dic,lv)  // wait for pre-write lock to be granted.  The DIC may have been resized during the wait, so pointers and limits must be refreshed after the lock
 ...
 I vb=dic->bloc.vbytelen; C *empty=CAV2(dic->bloc.empty); I emptyx=dic->bloc.emptyn, cur;  // empty area & current pointer into it.  Length of a element is hsz>>56
 for(cur=nroot;cur>=0;){
 ...
 for(cur=croot;cur>=0;){
  ...
  if(hval<4){
   ...
   emptyx=dic->bloc.emptyn; I emptynxt=_bzhi_u64(*(UI4*)&empty[emptyx*(hsz>>56)],(hsz>>53)); if(emptynxt==emptyx)goto resize; hval=emptyx+4; dic->bloc.emptyn=emptynxt; // get empty slot, save as biased kv slot#
 ...
 C *vbase=CAV(dic->bloc.vals)-4*vb;   // base pointer to allocated values, backed up to skip over the empty/birthstone/tombstone codes
 for(cur=oroot;cur>=0;){I nxt=((UI4*)(&s[cur]))[1]; UI8 kvslot=((UI4*)(&s[cur]))[0]; MC(vbase+kvslot*vb,(void*)((I)v+cur*vb),vb); cur=nxt;} // chase the old keys, copying the value
 for(cur=croot;cur>=0;){I nxt=((UI4*)(&s[cur]))[1]; UI8 kvslot=((UI4*)(&s[cur]))[0]; MC(vbase+kvslot*vb,(void*)((I)v+cur*vb),vb); cur=nxt;} // chase the conflict keys, copying the value
...
DF1(jtdicdelc){F12IP;
 ...
 ARGCHK1(w)
 R fdef(0,CIBEAM,VERB, jtdicdel,jtvalenceerr, w,0,0, VFLAGNONE, RMAX,RMAX,RMAX); 
}
...
DF1(jtdicgetc){F12IP;
 // We must not anticipate any values about the Dic because they may change during a resize and will not be visible to threads that have not taken a lock on the Dic
 ARGCHK1(w)
 R fdef(0,CIBEAM,VERB, jtdicget,jtvalenceerr, w,0,0, VFLAGNONE, RMAX,RMAX,RMAX); 
}
...
DF1(jtdicputc){F12IP;
 // We must not anticipate any values about the Dic because they may change during a resize and will not be visible to threads that have not taken a lock on the Dic
 ARGCHK1(w)
 R fdef(0,CIBEAM,VERB, jtvalenceerr,jtdicput, w,0,0, VFLAGNONE, RMAX,RMAX,RMAX); 
}
#endif