...
F2(jtfromis){A ind,x,z;B*b;I acr,af,an,ar,*av,k,m,*v,wcr,wf,wr,*ws,wt;P*wp,*zp;
 ...
 m=wcr; DO(wcr, m-=b[wf+i];); RZ(x=irs2(ind,SPA(wp,x),FUNCTYPE0,ar,m,jtifrom));
...
static A jtfrombs1(J jt,A ind,A w,I wf){A*iv,x,y,z;I j,m,n,wr,wcr;
 ...
 for(j=0;j<n;++j){
  ...
  RZ(z=irs2(x,z,FUNCTYPE0, RMAX,wcr-j,jtfromis)); z=gc(z,old);
 }
 R z;
}    /* (<ind){"r w, sparse w */
...
F2(jtfromsd){A e,x,z;I acr,af,ar,*v,wcr,wf,wr,*ws;P*ap,*zp;
 ...
 ar=AR(a); acr=jt->ranks>>RANKTX; acr=ar<acr?ar:acr; af=ar-acr;
 wr=AR(w); wcr=(RANKT)jt->ranks; wcr=wr<wcr?wr:wcr; wf=wr-wcr; RESETRANK;
 if(af)R sprank2(a,w,FUNCTYPE0,acr,wcr,jtfrom);
 ...
 RZ(x=irs2(SPA(ap,e),w,FUNCTYPE0,0L,wcr,jtifrom)); RZ(e=reshape(mtv,x));
 ASSERT(all1(eq(e,x)),EVSPARSE);
 SPB(zp,e,e);
 SPB(zp,a,wf?plus(sc(wf),SPA(ap,a)):SPA(ap,a));
 SPB(zp,i,SPA(ap,i));
 if(wf){
  RZ(x=irs2(SPA(ap,x),w,FUNCTYPE0, RMAX,wcr,jtifrom));
...
F2(jtfromss){A e,x,y,z;B*b;I acr,af,ar,c,d,k,m,n,p,*u,*v,wcr,wf,wr,*ws,*yv;P*ap,*wp,*xp,*zp;
 ...
 v=AS(z); ICPY(v+wf,AS(a),ar); if(wcr)ICPY(v+wf+ar,1+wf+ws,wcr-1);
 RZ(x=irs2(SPA(ap,e),w,FUNCTYPE0,0L,wcr,jtfrom)); RZ(e=reshape(mtv,x));
 ...
 SPB(zp,a,x);
 RZ(x=irs2(SPA(ap,x),w,FUNCTYPE0, RMAX,wcr,jtfrom)); xp=PAV(x); 
