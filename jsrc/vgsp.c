...
static A jtgrd1spsd(J jt,A w,I wf,I wcr){F1PREFJT;A d,t,y,z;I*dv,i,n,p,*tv,yc,*ws,*ys,*yv,*zv;P*wp;
 ...
 RZ(z=grd1spz(w,wf,wcr)); zv=AV(z);
 A spax=SPA(wp,x); RZ(t=IRS1(jtgr1,jtfg,spax,wcr,0)); tv=AV(t);  // grade dense cells - the flags are sort flags, not inplacing
 RZ(d=apvwr(wf,0L,0L)); dv=AV(d);                 /* odometer for frame             */
...
static A jtgrd1spdd(J jt,A w,I wf,I wcr){F1PREFJT;A x,z;I n,*ws;P*wp;
 wp=PAV(w); ws=AS(w); n=wcr?ws[wf]:1;
 x=SPA(wp,x);
 if(AN(x)){RZ(z=from(num(0),x)); R IRS1(jtgr1,jtfg,z,wcr,0);}else{R reshape(vec(INT,1+wf,ws),IX(n));}
...
// obsolete  wr=AR(w); wcr=(RANKT)jt->ranks; wcr=wr<wcr?wr:wcr; RESETRANK;
 ...
 m=0; j=wr; b=c=0; 
 DO(wf, if(wb[i])++m;); if(1<=m&&m<wf){c=1; mvc(wf,wb,MEMSET01LEN,MEMSET01);}
