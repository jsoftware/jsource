...
static I hashallo(IH * RESTRICT hh,UI p,UI m,I md){
 ...
 if(!(md&IIMODFORCE0)){
  ...
  // in the amount of (new left index end-right index end-hiprec m)*(width-currenthi).  We estimate hiprec values to equal current ones.
 ...
 if(mk){f=af; s=as; r=acr-1; f1=wcr-r;}  // if w is omitted (for prehashing), use info from a
  ...
  if(0>f1||ICMP(as+af+1,ws+wf+f1,r)){I f0,*v;
   ...
   switch(mode){
    ...
    case IEPS:    GATV(z,B01,zn,f+f0,s); if(af)MCISH(f+AS(z),ws+wf,f0); mvc(zn,BAV(z),1,MEMSET00); R z;
    case ILESS:                              RCA(w);
    case IIFBEPS:                            R mtv;
