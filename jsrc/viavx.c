...
#define RTNLINE (BIT8(INTX*4)+BIT8(INT2X*4)+BIT8(INT4X*4)+2*BIT8(FLX*4)+3*BIT8(CMPXX*4)+4*BIT8(QPX*4))   // line# for each type (0 for chars or INT[24])
  ...
  if(!p){  // If we selected small-range hashing, we have decided everything by analyzing the input; keep that.  Otherwise choose m or c depending on fwd/reverse hash
   ...
   if(((m>>1)>c) && (mode&IIOREPS) && fntbl[FNTABLEPREFIX+fnx+FNTBLREVERSE][0]){
    p=c; fnx+=FNTBLREVERSE; jtfg=(J)((I)jtfg&~JTINPLACEW);  // we mustn't inplace a reverse hash
...
F2(jtless){F12IP;A x=w;I ar,at,k,r,*s,wr,*ws;
 ...
 if(unlikely(ar==wr+1)){  // is just 1 cell of y, with x a list of such cells?
  if(wr==0)x=ne(a,w); else x=IRS2(jtnotmatch,jt,a,wr,w,wr,0); RZ(x) RZ(x=jtrepeat(jtfg,x,a,0))  // y has rank 1 less than x, execute as ((x ~: y) # x) (y atomic) or ((x -.@-:"yr) # x) if (y array).  Inplace x on the #.  Use IRS and leave comparison tolerance as set
 }else{
...
DF2(jtintersect){F12IP;A x=w;I ar,at,k,r,*s,wr,*ws;
 ...
 if(unlikely(ar==wr+1)){  // is just 1 cell of y, with x a list of such cells?
  if(wr==0)x=eq(a,w); else x=IRS2(jtmatch,jt,a,wr,w,wr,0); RZGOTO(x,errexit) x=jtrepeat(jtfg,x,a,0);  // y has rank 1 less than x, execute as ((x = y) # x) (y atomic) or ((x -:"yr) # x) if (y array).  Inplace x on the #.  Use IRS and leave comparison tolerance as set
 }else{
  // if w's rank is larger than that of a cell of a, reheader w to look like a list of such cells
...
A jtiocol(J jt,I mode,A a,A w){A h,z;I ar,at,c,d,m,t,wr,*ws,wt;IOCOLFN fn;
 ARGCHK2(a,w);
 // require ct!=0   why??
