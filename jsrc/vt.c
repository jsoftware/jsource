...
// take empty or from empty, u=take shape (length n), w=takefrom. empty=1 if result is empty (i. e. an axis in u, or an axis in w outside of u, is 0).  istake is 1 if take, 0 if drop
static A jttk0(J jt,B empty,I *u, I n,A w, I wf, I istake){A z;I m,r,*s;DPMULDDECLS
 ...
 if(!empty){  // w was empty but the result is nonempty.  Can happen only for take.
  I p=1; DO(n, I m=ABS(u[i]); if(unlikely(m==IMAX))m=ws[wf+i]; DPMULDE(p,m,p)) PRODX(p,r-(wf+n),&AS(w)[wf+n],ABS(p)) PRODX(m,wf,AS(w),p)  // count atoms, applying infinities.  We have values from a, then tail of w, then frame of w
...
DFI1(jttail){
 ...
 if(unlikely(!wcr)){RETF(RETARG(w))  // {:"0, a NOP
  ...
  // Use from.  With IRS, so this may produce multiple cells
  // left rank is garbage, but since num(-1) is an atom it doesn't matter
  RETF(jtfrom(jtfg,num(-1),wfg,ds(CFROM)));  // scaf could call jtfromi directly for non-sparse w
 }else{RETF(ISSPARSE(AT(w))?irs2(zeroionei(0),take(num(-1),wfg),0L,0L,wcr,jtfrom):rsh0(wfg));  // sparse or cell of w is empty - create a cell of fills   Left rank is garbage, but that's OK
 }
