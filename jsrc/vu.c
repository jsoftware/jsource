...
static F1(jttoc2){F12IP;A z;C*wv,*zv;I n;C4*w4;US*z2;
 ARGCHK1(w);
 if(C2T&AT(w))RCA(w);
...
static F1(jttoc2e){F12IP;A z;I m,n,r;
 ARGCHK1(w);
 n=AN(w); r=AR(w);
 ASSERT(r!=0,EVRANK);
...
// extended to C4
static F1(jtifc2){F12IP;A z;I n,t,*zv;
 ARGCHK1(w);
 n=AN(w); t=AT(w);
...
static F1(jtc2fi){F12IP;A z;I j,n,*v;US*zv;
 RZ(w=vi(w));
 n=AN(w); v=AV(w);
