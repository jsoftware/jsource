...
// obsolete  if((env==2)&&(NAV(*x)->flag&NMXY)){AT(*x)|=NAMEBYVALUE;}
...
// obsolete  if(unlikely((env==2)&&(NAV(*x)->flag&NMXY))){AT(*x)|=NAMEBYVALUE;}     // If the name is a call-by-value name (x y u. etc), we mark it as BYVALUE if it is slated for execution in an explicit definition
