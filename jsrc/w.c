 ...
 *x=i;
 m=((x-AV(z))+1)>>1; AS(z)[0]=m; AM(z)=m-(SHMSK((1LL<<SNZ)|(1LL<<SZ),s>>4,1)); AN(z)=m<<1; // Calculate & install count; if last field is NB., subtract 1 from word count
 if(unlikely((s>>4)==SQ))AM(z)=-1;  // error if open quote, indicated by neg AM
 R z;
...
A jtenqueue(J jt,A a,A w,I env){A*v,*x,y,z;B b;C d,e,p,*s,*wi;I i,n,*u,wl;UC c;
 ...
 ASSERT(!(n==1&&!(AT(QCWORD(v[0]))&NAME+CONJ+ADV+VERB+NOUN)),EVSYNTAX)  // single word must not be anything that can't be a result
