...
static I conend(I i,I j,I k,CW*b,CW*c,CW*d,I p,I q,I r,CW*con){I e,m,t;
 ...
 if(BOTHEQUS(CIF,CDO,bothcw));  // if. do. end. - nothing
  ...
  DQ(m, ++d; t=(d->tcesx>>TCESXTYPEX); if(CWMAX==d->go)d->go=SHMSK((BIT(CBREAK)|BIT(CBREAKS)),t,1)?(US)e :SHMSK(BIT(CCONT)|BIT(CCONTS),t&31,1)?(US)(1+k):(US)CWMAX;);
  ...
  DQ(m, ++d; t=(d->tcesx>>TCESXTYPEX); if(CWMAX==d->go)d->go=SHMSK(BIT(CBREAK)|BIT(CBREAKS),t,1)?(d->tcesx=(d->tcesx&~TCESXTYPE)|(CBREAKF<<TCESXTYPEX),(US)e):SHMSK(BIT(CCONT)|BIT(CCONTS),t&31,1)?(US)j:(US)CWMAX;);  // replace 'goto CWMAX' with actual end value
 }else CWASSERT(0);
...
static I conendtry(I e,I top,I stack[],CW*con){CW*v;I c[3],d[4],i=-1,j,k=0,m,t=0;US ii;
 ...
 if     (0<=c[0]){ii=(US)(1+c[0]); v=j+con; DQ(m-j-1, ++v; if(CWMAX==v->go&&!PEXT0(BIT(CCONT)|BIT(CCONTS)|BIT(CBREAK)|BIT(CBREAKS),PEXT0(v->tcesx,TCESXTYPEX,31),1))v->go=ii;);}
 else if(0<=c[1]){ii=(US)(1+c[1]); v=j+con; DQ(m-j-1, ++v; if(CWMAX==v->go&&!PEXT0(BIT(CCONT)|BIT(CCONTS)|BIT(CBREAK)|BIT(CBREAKS),PEXT0(v->tcesx,TCESXTYPEX,31),1))v->go=ii;);}
...
static I jtconall(J jt,I n,CW*con){A y;CW*b=0,*c=0,*d=0;I e,i,j,k,p=0,q,r,*stack,tb=0,top=0,wb=0;
 ...
 for(i=0;i<n;++i){
  ...
  case CDO:                                   // do. - classify according to what structure we are in
   CWASSERT(SHMSK(BIT(CIF)|BIT(CELSEIF)|BIT(CSELECT)|BIT(CWHILE)|BIT(CWHILST)|BIT(CFOR)|BIT(CCASE)|BIT(CFCASE),q&31,1));   // first, make sure do. is allowed here
