...
A jtconnum(J jt,I n,C*s){PROLOG(0101);A y,z;B (*f)(J,I,C*,void*),p=1;C c,*v;I d=0,e,k,m,t,*yv;
 ...
 if(SHMSK(AT(z),FLX,1)>AR(z)){D zv=DAV(z)[0]; z=zv==DAV0(onehalf)[0]?onehalf:z; z=zv==DAV0((A)&Bnumvr[1])[0]?(A)&Bnumvr[1]:z; z=zv==DAV0((A)&Bnumvr[2])[0]?(A)&Bnumvr[2]:z; z=*(UI8*)&zv==*(UI8*)&DAV0(ds(CUSDOT))[0]?ds(CUSDOT):z;}  // nan always fails fl comparison
 else if(SHMSK(AT(z),INTX,1)>AR(z)){I zv=IAV(z)[0]; z=zv&~1?z:zeroionei(zv);}  // int 0 & 1
 EPILOG(z);
}
