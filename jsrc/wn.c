...
A jtconnum(J jt,I n,C*s){PROLOG(0101);A y,z;B (*f)(J,I,C*,void*),p=1;C c,*v;I d=0,e,k,m,t,*yv;
 ...
 if(PEXT0(AT(z),FLX,1)>AR(z)){D zv=DAV(z)[0]; z=zv==DAV0(onehalf)[0]?onehalf:z; z=zv==DAV0((A)&Bnumvr[1])[0]?(A)&Bnumvr[1]:z; z=zv==DAV0((A)&Bnumvr[2])[0]?(A)&Bnumvr[2]:z; z=*(UI8*)&zv==*(UI8*)&DAV0(ds(CUSDOT))[0]?ds(CUSDOT):z;}  // nan always fails fl comparison
 else if(PEXT0(AT(z),INTX,1)>AR(z)){I zv=IAV(z)[0]; z=zv&~1?z:zeroionei(zv);}  // int 0 & 1
 EPILOG(z);
}
...
// x ". y
F2(jtexec2){F12IP;A z;B b,p;C d,*v;I at,c,i,k,m,n,r,*s;
 ARGCHK2(a,w);
 ...
 if(!r||AS(w)[r-1]){    // skip the count if y is atom, or the last axis of y has dimension 0.   Nothing to count.
  ...
  {RZ(w=IRS2(jtover,jt,w,1L,chrspace,0L,NOEMSGSELF)); makewritable(w);}  // New w will be created
