...
F2(jtaes2)
{
  ...
  ASSERT(AT(a)&BOX,EVDOMAIN);ASSERT(1>=AR(a),EVRANK);ASSERT(AN(a)>=3&&AN(a)<=4,EVLENGTH);
  av=AAV(a);ASSERT(1>=AR(av[0]),EVRANK);RZ(dec=vi(av[0]));ASSERT(AN(dec)==1,EVDOMAIN);decrypt=(AV(dec))[0];ASSERT(decrypt==0||decrypt==1,EVDOMAIN);
  ASSERT(AT(av[1])&LIT,EVDOMAIN);ASSERT(1>=AR(av[1]),EVRANK);key=UAV(av[1]);keyn=AN(av[1]);ASSERT(keyn==16||keyn==24||keyn==32,EVDOMAIN);
  ASSERT(AT(av[2])&LIT,EVDOMAIN);ASSERT(1>=AR(av[2]),EVRANK);iv=UAV(av[2]);ASSERT(AN(av[2])==16,EVDOMAIN);
  if(AN(a)>3){ASSERT(AT(av[3])&LIT,EVDOMAIN);ASSERT(1>=AR(av[3]),EVRANK);mode=(!strncasecmp(CAV(av[3]),"ECB",AN(av[3])))?0:(!strncasecmp(CAV(av[3]),"CBC",AN(av[3])))?1:(!strncasecmp(CAV(av[3]),"CTR",AN(av[3])))?2:-1;ASSERT(mode!=-1,EVDOMAIN);}
  n=AN(w);
