...
static UINT jtcrcvalidate(J jt,A w, UINT* crctab){A*wv;B*v;I m;UINT p,x,z=-1;
 ...
 m=AN(w);
 if(m&&BOX&AT(w)){ASSERT(2>=m,EVLENGTH); wv=AAV(w);  w=C(wv[0]); if(2==m)z=(UINT)rei0(C(wv[1]));}
...
DF1(jtfrombase64){F12IP;
 ...
 for(wn-=3;wn>0;wn-=4){  // for each block of 4, not counting an incomplete last one
  ...
  UI4 bytes4=*wv4++; I ba=base64invtab[bytes4&0xff]; I bb=base64invtab[PEXT0(bytes4,8,0xff)];  I bc=base64invtab[PEXT0(bytes4,16,0xff)];  I bd=base64invtab[PEXT0(bytes4,24,0xff)];
...
DF2(jtxxhash2){F12IP;
  ...
  A z;
  UC *v;
  if(AT(w)&NOUN){s=rei0(a);} else {w=a; s=4;}
  ASSERT(!ISSPARSE(AT(w)),EVNONCE);  // not sparse for now
