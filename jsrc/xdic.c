...
F1(jtcreatedic){F12IP;
 ARGCHK1(w)
 R fdef(0,CIBEAM,VERB,jtcreatedic1,jtvalenceerr,w,0,0,VNONAME+VNOSELF,RMAX,RMAX,RMAX);  // Must mark VNONAME+VNOSELF to void reinterpreting the dic during f.
}
...
#define DICLKRDRQ(dic,lv,cond) (lv=DICLSINGLETHREADED)
...
#define DICLKRWRQ(dic,lv,cond) lv=DICLSINGLETHREADED;
...
static INLINE UI8 jtputslots(DIC *dic,void *k,I n,void *v,I vn,I8 *s,J jt,UI lv,VIRT virt){I i;
 ...
 DQ(n, I sh=((UI4*)(&s[i]))[1]; if(unlikely(sh&(BIT(HASHBSTN)+BIT(HASHTSTN+HASHBSTN)))){((UI4*)(&s[i]))[1]=croot; croot=i;}else{((I4*)(&s[i]))[1]=sh?nroot:oroot; nroot=sh?i:nroot; oroot=sh?oroot:i; nnew+=sh!=0; otail=sh|otail?otail:i;})
...
// del fast case: one key, no user hash or compare function.  We enter having requested a pre-write lock.
...
notfound:;  // key not found.  lv has DICLMSKWRK set; we clear that as an indicator of notfound.  We don't bother waiting for read locks to complete
 R (lv|DICLMSKOKRET)&~DICLMSKWRK;  // we always have a lock on the keys, so we can turn the bit off as a flag.  The caller will reinstate the lock bit

// eventually resize:;  // resize required.  We already have a write lock
// eventually R lv|DICLMSKOKRET+DICLMSKRESIZEREQ;  // signal need for resize; return holding write lock on keys & values
}
...
static DF1(jtdicdel){F12IP;A z;
 ...
 if((wf+((dic->bloc.flags&DICFIHF+DICFICF)^(DICFIHF+DICFICF)))==0){  // fast path?
  ...
  // We always have a lock on keys when we get here (but we may not have waited for read locks to disappear).  We turned off the write-lock as a flag to indicate key not found.
  if(likely(lv&DICLMSKWRK)){z=num(1);}else{z=num(0); lv|=DICLMSKWRK;}  // set return based on found status, reinstate known write lock (not required but saves a pass through the CAS loop)
 }else{
...
#if defined(RASPI) && !SY_64   // 32-bit Rpi has optimization error that loses a value
...
//  2 - no compare (infer direction from 1)
//  4 - no stack
...
static I searchtree(I type,J jt,C *hashtbl, UI8 kib, I nodeb, C *kbase, VIRT virt, I (*cf)(I,void*,void*), UI4 sp[][2], I *flags, C *k){
 ...
 if(!(type&4))sp[0][0]=0;   // top of stack is the empty-tree pointer, when stack is being created
 res1[2-2*(type&1)]=0;  // init 'stack at first key' to 0 (invalid).  If it is still there after the search there were no valid keys
 I zx=type&4?0:1;  // next fill pointer in sp - if no stack, will remain at 0
 ...
 do{  // traverse the tree, searching for index k.  Current node is nodex
  ...
  if(!(type&2)){  // normal case, do compare
   comp=keysne((UI4)kib,kbase+(kib>>32)*(nodex>>1),k,nodeb&(DICFICF<<8),errexit);  // compare node key vs k (key to be found), so node > k is 1
   if(comp==0)R type&4?nodex:zx;  // found at node nodex.  sp points to the children
  }else{  // comparison forced
   comp=1-2*(type&1);  // if L-to-R, stack R and go to L descendant: comp=1
  }
  // No match.  drop down to next node.
  if(type&4)res1[1+comp]=nodex; else res1[1+comp]=zx++;  // remember sp of the first result in res1[1 or 2].  Stores to res[0] are wasted.   advance sp to next empty slot, which we will fill presently
 ...
 *flags|=type&4?MGETFLGEQ1:MGETFLGEQ0;   // indicate that we should include appropriate endpoint, which does not match the key
...
endscan:;   // come here when we encounter the end node, possibly without looking at anything
...
#define S1RLCMP 1
#define S1RLMAX 3
#define PLISTS2X 2   // bit# of command
#define S2CMP (0<<PLISTS2X)   // routine index after shift
#define S2LR (1<<PLISTS2X)
#define S2RL (2<<PLISTS2X)
...
static INLINE UI8 jtdelslotso(DIC *dic,void *k,I n,J jt,UI lv,VIRT virt,B *zv){I i;
 ...
 C *hashtbl=CAV3(dic->bloc.hash);  // pointer to tree base
 C *kbase=CAV(dic->bloc.keys)-TREENRES*(kib>>32), *kbasei=CAV(dic->bloc.keys);  // address corresponding to tree value of 0.  treevalues 0-1 are empty and do not take space in the key array
 ...
 for(i=n-1;i>=0;--i){I nodex;
  ...
  DELKV(kbasei+emptyx*(kib>>32),kib>>32,nodeb&(DICFKINDIR<<8)) DELKV(vbase+emptyx*vb,vb,nodeb&(DICFVINDIR<<8))   // if k/v is indirect, free it & clear to 0
  I t=dic->bloc.emptyn; WRHASH1234(t, nodeb>>19, &kbasei[emptyx*(kib>>32)])  // chain old free chain from new deletion
...
DF1(jtdicgetc){F12IP;
 ...
 A z; fdefallo(z);
 fdeffillall(z,0,CMODX,VERB, !(((DIC*)w)->bloc.flags&DICFRB)?jtdicget:jtdicgeto,!(((DIC*)w)->bloc.flags&DICFRB)?jtdicget:jtdicgeto,w,self,0, VNONAME+VNOSELF, RMAX,RMAX,RMAX,fffv->localuse.lu0.cachedloc=0,FAV(z)->localuse.lu1.varno=0);
 R z;
}
...
DF1(jtdichasc){F12IP;
 ...
 ASSERT(AT(w)&NOUN,EVDOMAIN)  // must be a dic.  Perhaps we should demand credentials
 A z; fdefallo(z);
 fdeffillall(z,0,CMODX,VERB, !(((DIC*)w)->bloc.flags&DICFRB)?jtdicget:jtdicgeto,jtvalenceerr,w,self,0, VNONAME+VNOSELF, RMAX,RMAX,RMAX,fffv->localuse.lu0.cachedloc=0,FAV(z)->localuse.lu1.varno=1);
 R z;
}
...
DF1(jtdicdelc){F12IP;
 ...
 ARGCHK1(w)
 ASSERT(AT(w)&NOUN,EVDOMAIN)  // must be a dic.  Perhaps we should demand credentials
 R fdef(0,CMODX,VERB, !(((DIC*)w)->bloc.flags&DICFRB)?jtdicdel:jtdicdelo,jtvalenceerr, w,self,0, VNONAME+VNOSELF, RMAX,RMAX,RMAX); 
}
...
static I initrb(I min,I max,I currdepth,I minreddepth,C *hashtbl,I nodeb){
 if(min>max)R 0;  // min<max is an empty interval: return empty pointer
...
// (keys;values) 16!:_10 dic   Install sorted keys/values into red/black tree en bloc
// No locks.
DF2(jtdicload){F12IP;
 ARGCHK2(a,w)
 ...
 // copy in keys
 UI8 hsz=dic->bloc.hashsiz; UI8 kib=dic->bloc.klens; UI4 (*hf)(void*,I,J)=dic->bloc.hashfn; C *hashtbl=CAV3(dic->bloc.hash);  // elesiz/hashsiz kbytelen/kitemlen
 I nodeb=dic->bloc.hashelesiz*(0x1000000+BB)+(dic->bloc.emptysiz<<19)+(dic->bloc.flags<<8);  // (#bytes in node index)\(#bits in empty-chain field\(flags)\(number of bits in a node index)
 C *kbase=CAV(dic->bloc.keys);  // key 0 address for empties; Hashvalues 0-3 are empty/tombstone/birthstone and do not take space in the key array
